...
TOP_CACHE = "/static/js/modules/top_menu_active_v1.js?v=20260505-bloquear-sessoes-sidebar-legado-v39"
SIDEBAR_CACHE = "/static/js/modules/sidebar_sections_layout_v1.js?v=20260505-bloquear-sessoes-sidebar-legado-v39"
ADMIN_CSS_CACHE = "/static/css/modules/admin_subprocesses_v1.css?v=20260505-bloquear-sessoes-sidebar-legado-v39"
ADMIN_JS_CACHE = "/static/js/modules/admin_subprocesses_v1.js?v=20260505-bloquear-sessoes-sidebar-legado-v39"

TEMPLATE_MARKER_START = "<!-- APPVERBO_TOPCENTRAL_ACIMA_SESSOES_V39_START -->"
TEMPLATE_MARKER_END = "<!-- APPVERBO_TOPCENTRAL_ACIMA_SESSOES_V39_END -->"
...
def fail_v39(message: str) -> None:
    print(f"ERRO: {message}")
    sys.exit(1)


def read_text_v39(path: Path) -> str:
    return path.read_text(encoding="utf-8-sig")


def write_text_v39(path: Path, content: str) -> None:
    path.write_text(content.rstrip() + "\n", encoding="utf-8")


def normalize_v39(value: str) -> str:
    text = str(value or "")
...
def strip_tags_v39(html: str) -> str:
    return re.sub(r"<[^>]+>", " ", html)


def remove_html_marker_family_v39(content: str, family: str) -> str:
    pattern = re.compile(
...
def remove_js_marker_family_v39(content: str, family: str) -> str:
    pattern = re.compile(
...
def find_main_container_open_end_v39(content: str) -> int:
    ...
    if not main_match:
        fail_v39('não foi encontrado <main class="content"> em templates/new_user.html')
    ...
    if not container_match:
        fail_v39('não foi encontrado <div class="container"> dentro de <main class="content">')

    return main_match.end() + container_match.end()


def find_menu_tabs_sections_v39(content: str):
    pattern = re.compile(
...
def find_section_bounds_by_id_v39(content: str, section_id: str):
    ...
    fail_v39(f"section sem fechamento: {section_id}")


def remove_section_by_id_v39(content: str, section_id: str) -> str:
    bounds = find_section_bounds_by_id_v39(content, section_id)
...
def remove_sections_containing_v39(content: str, terms: list[str]) -> str:
    section_pattern = re.compile(r'\n?[ \t]*<section\b[\s\S]*?</section>[ \t]*\n?', re.I)

    def repl(match: re.Match) -> str:
        section_html = match.group(0)
        raw = section_html.lower()
        clean = normalize_v39(strip_tags_v39(section_html))

        for term in terms:
            term_raw = str(term or "").lower()
            term_clean = normalize_v39(term)
...
def remove_existing_sessoes_renderer_v39(content: str) -> str:
    content = re.sub(
...
def ensure_macro_import_v39(content: str) -> str:
...
def ensure_head_asset_v39(content: str, asset_html: str, asset_name: str) -> str:
    content = re.sub(
    ...
    if not head_match:
        fail_v39(f"não encontrei bloco head_extra para inserir {asset_name}")

    insert_at = head_match.start(3)
    return content[:insert_at] + "  " + asset_html + "\n" + content[insert_at:]


def ensure_topcentral_first_v39(content: str) -> str:
    tabs_sections = find_menu_tabs_sections_v39(content)
    ...
    container_open_end = find_main_container_open_end_v39(without_tabs)
...
def insert_sessoes_after_topcentral_v39(content: str) -> str:
    bounds = find_section_bounds_by_id_v39(content, "menu-tabs-card")

    if not bounds:
        fail_v39("menu-tabs-card ausente.")
...
for file_path in [TEMPLATE_PATH, SIDEBAR_JS_PATH, TOP_MENU_JS_PATH]:
    if not file_path.exists():
        fail_v39(f"ficheiro não encontrado: {file_path}")
...
template = read_text_v39(TEMPLATE_PATH)
...
for family in [
    ...
    template = remove_html_marker_family_v39(template, family)

template = remove_existing_sessoes_renderer_v39(template)
...
for section_id in [
    ...
    template = remove_section_by_id_v39(template, section_id)

template = remove_sections_containing_v39(
    template,
    [
...
template = ensure_macro_import_v39(template)
template = ensure_topcentral_first_v39(template)
template = insert_sessoes_after_topcentral_v39(template)

template = ensure_head_asset_v39(
    template,
...
write_text_v39(TEMPLATE_PATH, template)

print("OK: template corrigido.")
...
sidebar_js = read_text_v39(SIDEBAR_JS_PATH)
...
for family in [
    ...
    sidebar_js = remove_js_marker_family_v39(sidebar_js, family)
...
hard_stop_block = '''// APPVERBO_SESSOES_HARD_STOP_LEGADO_V39_START
...
if '"use strict";' not in sidebar_js:
    fail_v39('não encontrei "use strict"; em sidebar_sections_layout_v1.js')
...
write_text_v39(SIDEBAR_JS_PATH, sidebar_js)

print("OK: sidebar_sections_layout_v1.js com hard stop V39.")
...
top_js = read_text_v39(TOP_MENU_JS_PATH)
...
for family in [
    ...
    top_js = remove_js_marker_family_v39(top_js, family)
...
write_text_v39(TOP_MENU_JS_PATH, top_js)
...
for template_file in (ROOT / "templates").rglob("*.html"):
    content = read_text_v39(template_file)
    original = content
    ...
    if content != original:
        write_text_v39(template_file, content)
...
template_validado = read_text_v39(TEMPLATE_PATH)
sidebar_validado = read_text_v39(SIDEBAR_JS_PATH)
top_validado = read_text_v39(TOP_MENU_JS_PATH)

tabs = find_menu_tabs_sections_v39(template_validado)

if len(tabs) != 1:
    fail_v39(f"deve existir exatamente 1 menu-tabs-card; encontrado: {len(tabs)}")

if TEMPLATE_MARKER_START not in template_validado:
    fail_v39("marcador V39 ausente no template.")

tabs_bounds = find_section_bounds_by_id_v39(template_validado, "menu-tabs-card")

if not tabs_bounds:
    fail_v39("menu-tabs-card ausente no template.")

_, tabs_end = tabs_bounds
sessoes_marker = template_validado.find(TEMPLATE_MARKER_START)

if sessoes_marker < tabs_end:
    fail_v39("bloco de Sessões está antes do TopCentral.")

container_open_end = find_main_container_open_end_v39(template_validado)
between_container_and_tabs = template_validado[container_open_end:tabs[0].start()].strip()

if between_container_and_tabs:
    fail_v39("menu-tabs-card não está como primeiro elemento do container central.")

template_clean = normalize_v39(strip_tags_v39(template_validado))

if "sessoes do sidebar" in template_clean:
    fail_v39("texto legado 'Sessões do sidebar' ainda existe no template.")
...
for forbidden in [
    ...
    if forbidden in template_validado:
        fail_v39(f"conteúdo visual legado ainda existe no template: {forbidden}")

if "APPVERBO_SESSOES_HARD_STOP_LEGADO_V39_START" not in sidebar_validado:
    fail_v39("hard stop V39 ausente em sidebar_sections_layout_v1.js.")

if NEW_SESSOES_URL not in top_validado:
    fail_v39("URL correta de Sessões ausente no top_menu_active_v1.js.")

if 'menu: "/users/new?menu=administrativo&admin_tab=menu"' not in top_validado:
    fail_v39("URL correta de Menu ausente no top_menu_active_v1.js.")

for file_path in [TEMPLATE_PATH, SIDEBAR_JS_PATH, TOP_MENU_JS_PATH]:
    content = read_text_v39(file_path)

    if not content.endswith("\n"):
        fail_v39(f"ficheiro sem newline final: {file_path}")

    if content.endswith("\n\n"):
        fail_v39(f"ficheiro com linha em branco extra no EOF: {file_path}")

print("OK: validações V39 concluídas.")
