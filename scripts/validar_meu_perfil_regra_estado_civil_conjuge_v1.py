...
EXPECTED_OPERATOR = "not_equals"
EXPECTED_TRIGGER_VALUE = "Solteiro"
...
def normalize_lookup_text_v1(value: Any) -> str:
...
def is_rule_met_v1(operator: str, current_value: Any, trigger_value: Any) -> bool:
    clean_operator = normalize_key_v1(operator)

    current_text = str(current_value or "").strip()
    trigger_text = str(trigger_value or "").strip()

    normalized_current = normalize_lookup_text_v1(current_text)
    normalized_trigger = normalize_lookup_text_v1(trigger_text)
...
####################################################################################
# (5) EXTRAIR CAMPOS E REGRAS DO MENU
####################################################################################

def extract_field_metadata_v1(menu_config: dict[str, Any]) -> dict[str, Any]:
    ...
    for raw_field in additional_fields_raw:
        ...
        is_required_raw = normalize_key_v1(raw_field.get("is_required"))
...
def normalize_rule_v1(raw_rule: Any, index: int, source_key: str) -> dict[str, str]:
    ...
    trigger_field = normalize_key_v1(
...
def extract_subsequent_rules_v1(menu_config: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for source_key in SUBSEQUENT_KEYS_TO_CHECK:
        ...
        for index, raw_rule in enumerate(raw_rules):
            rules.append(normalize_rule_v1(raw_rule, index, source_key))

    return rules


def find_estado_civil_conjuge_rule_v1(rules: list[dict[str, str]]) -> dict[str, str] | None:
    for rule in rules:
        if (
            rule.get("trigger_field") == TRIGGER_FIELD
            and rule.get("field_key") == TARGET_FIELD
            and rule.get("operator") == EXPECTED_OPERATOR
            and normalize_lookup_text_v1(rule.get("trigger_value")) == normalize_lookup_text_v1(EXPECTED_TRIGGER_VALUE)
        ):
            return rule
...
####################################################################################
# (6) VALIDAR DADOS GRAVADOS DOS MEMBROS
####################################################################################

def get_custom_field_value_v1(profile_custom_fields: dict[str, Any], field_key: str) -> str:
    clean_key = normalize_key_v1(field_key)
    value = profile_custom_fields.get(clean_key, "")
    return str(value or "").strip()


def classify_member_status_v1(
    estado_civil: str,
    nome_conjuge: str,
    rule: dict[str, str],
    target_required: bool,
) -> tuple[str, str]:
    should_show_target = is_rule_met_v1(
        rule.get("operator"),
        estado_civil,
    ...
    if should_show_target and has_nome_conjuge:
        return (
            ...
            "A regra manda mostrar Nome do conjuge e existe valor gravado.",
        )
    ...
    if should_show_target and not has_nome_conjuge:
        if target_required:
            return (
                ...
                "A regra manda mostrar Nome do conjuge, o campo é obrigatório, mas não existe valor gravado.",
            )

        return (
            "AVISO_VISIVEL_SEM_VALOR",
            "A regra manda mostrar Nome do conjuge, mas não existe valor gravado. Como o campo não está marcado como obrigatório, pode estar correto.",
        )

    if not should_show_target and has_nome_conjuge:
        return (
            "ERRO_OCULTO_COM_VALOR",
            "A regra manda ocultar Nome do conjuge, mas existe valor gravado.",
        )

    return (
        "OK_OCULTO_SEM_VALOR",
        "A regra manda ocultar Nome do conjuge e não existe valor gravado.",
    )
...
def main_v1() -> None:
    with SessionLocal() as session:
        ...
        field_metadata = extract_field_metadata_v1(menu_config)
        all_rules = extract_subsequent_rules_v1(menu_config)
        estado_civil_rule = find_estado_civil_conjuge_rule_v1(all_rules)
        ...
        print("")
        print("===== REGRA ESPERADA =====")
        print(f"Campo acionador: {TRIGGER_FIELD} - {field_metadata['labels_by_key'].get(TRIGGER_FIELD, TRIGGER_FIELD)}")
        print(f"Condição: {EXPECTED_OPERATOR} - {get_operator_label_v1(EXPECTED_OPERATOR)}")
        print(f"Valor acionador: {EXPECTED_TRIGGER_VALUE}")
        print(f"Campo subsequente: {TARGET_FIELD} - {field_metadata['labels_by_key'].get(TARGET_FIELD, TARGET_FIELD)}")

        print("")
        print("===== REGRA ENCONTRADA NA CONFIGURAÇÃO =====")

        if estado_civil_rule is None:
            print("ERRO: regra esperada não encontrada.")
            print("Nenhuma validação de dados dos membros será feita.")
            raise SystemExit(1)

        print(f"origem: {estado_civil_rule.get('source_key')}")
        print(f"key: {estado_civil_rule.get('key')}")
        print(f"trigger_field: {estado_civil_rule.get('trigger_field')}")
        print(f"operator: {estado_civil_rule.get('operator')} - {get_operator_label_v1(estado_civil_rule.get('operator', ''))}")
        print(f"trigger_value: {estado_civil_rule.get('trigger_value')}")
        print(f"field_key: {estado_civil_rule.get('field_key')}")
        ...
        for member_row in member_rows:
            profile_custom_fields = safe_json_loads_v1(member_row.get("profile_custom_fields"), {})

            if not isinstance(profile_custom_fields, dict):
                profile_custom_fields = {}

            estado_civil = get_custom_field_value_v1(profile_custom_fields, TRIGGER_FIELD)
            nome_conjuge = get_custom_field_value_v1(profile_custom_fields, TARGET_FIELD)

            status_code, status_message = classify_member_status_v1(
                estado_civil,
                nome_conjuge,
                estado_civil_rule,
                target_required,
            ...
            should_show_target = is_rule_met_v1(
                estado_civil_rule.get("operator"),
                estado_civil,
                estado_civil_rule.get("trigger_value"),
            )
        ...
        for row in validation_rows:
            ...
            print(f"- Estado civil gravado: {format_value_v1(row['estado_civil'])}")
            print(f"- Nome do conjuge gravado: {format_value_v1(row['nome_conjuge'])}")
            print(f"- Pela regra, Nome do conjuge deve aparecer? {row['should_show_target']}")
