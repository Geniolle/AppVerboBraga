...
def replace_function_v1(content: str, function_name: str, replacement: str) -> str:
    pattern = re.compile(
    ...
    if not pattern.search(content):
        raise RuntimeError(f"Função não encontrada: {function_name}")
...
def patch_menu_settings_v1() -> None:
    content = read_text_v1(MENU_SETTINGS_PATH)
...
def get_menu_process_visible_fields(
    ...
    # APPVERBO_PROCESS_FIELDS_EMPTY_DELETE_V1_START
    ...
    # APPVERBO_PROCESS_FIELDS_EMPTY_DELETE_V1_END
'''

    content = replace_function_v1(
        content,
...
def update_sidebar_menu_process_fields(
    ...
    # APPVERBO_PROCESS_FIELDS_EMPTY_DELETE_SAVE_V1_START
    clean_menu_key = _resolve_legacy_menu_alias(menu_key)
    ...
    return True, ""
    # APPVERBO_PROCESS_FIELDS_EMPTY_DELETE_SAVE_V1_END
'''
    ...
    write_text_v1(MENU_SETTINGS_PATH, content)
...
def patch_template_v1() -> None:
    content = read_text_v1(HTML_PATH)
    ...
    script_tag = '  <script src="/static/js/modules/process_fields_config_manager_v6.js?v=20260501-process-fields-config-v6-empty-delete-v1"></script>'
    ...
    write_text_v1(HTML_PATH, content)
...
def validate_v6_v1() -> None:
    content = read_text_v1(V6_PATH)
