...
SUBSEQUENT_FIELDS_PATH = SETTINGS_PACKAGE_DIR / "subsequent_fields_handlers.py"
...
MOVED_MARKER_START = "# APPVERBO_SETTINGS_HANDLERS_SUBSEQUENT_FIELDS_MOVED_V1_START"
MOVED_MARKER_END = "# APPVERBO_SETTINGS_HANDLERS_SUBSEQUENT_FIELDS_MOVED_V1_END"
...
def should_move_subsequent_fields_block_v1(block: str) -> bool:
    ...
    if "process-list" in route_path or "process-lists" in route_path:
        return False
    ...
    if "update_sidebar_menu_subsequent_fields" in block:
        return True

    if "subsequent_field" in block_lower or "subsequent_fields" in block_lower:
        return True
...
def extract_selected_router_blocks_v1(content: str) -> tuple[list[str], str, list[str]]:
    ...
    for block_start, block_end in ranges:
        ...
        if should_move_subsequent_fields_block_v1(block):
...
####################################################################################
# (3) MÓDULO DE CAMPOS SUBSEQUENTES
####################################################################################

def build_subsequent_fields_module_v1(blocks: list[str]) -> str:
...
####################################################################################
# (4) REFATORAR CAMPOS SUBSEQUENTES
####################################################################################
...
def patch_settings_handlers_v1() -> None:
    ...
    already_moved = "subsequent_fields_handlers_v1" in settings_content

    if already_moved:
        if not SUBSEQUENT_FIELDS_PATH.exists():
            raise RuntimeError(
                "settings_handlers.py já importa subsequent_fields_handlers_v1, "
                "mas o ficheiro subsequent_fields_handlers.py não existe."
            )

        print("INFO: Campos subsequentes já estavam importados.")
        return
    ...
    if not extracted_blocks:
        raise RuntimeError(
            "Nenhum endpoint de Campos subsequentes foi encontrado para mover. "
            "Confirme se ainda existem rotas/handlers de subsequent-fields em settings_handlers.py."
        )

    subsequent_fields_module = build_subsequent_fields_module_v1(extracted_blocks)
    write_text_v1(SUBSEQUENT_FIELDS_PATH, subsequent_fields_module)
...
# Campos subsequentes do Processo/Menu movidos para:
# appverbo/routes/profile/settings/subsequent_fields_handlers.py
...
def validate_v1() -> None:
    settings_content = read_text_v1(SETTINGS_HANDLERS_PATH)
    subsequent_fields_content = read_text_v1(SUBSEQUENT_FIELDS_PATH)

    required_settings_markers = [
        "APPVERBO_SETTINGS_SUBPROCESS_IMPORTS_V1_START",
        "subsequent_fields_handlers_v1",
        "APPVERBO_SETTINGS_HANDLERS_SUBSEQUENT_FIELDS_MOVED_V1_START",
    ]

    required_subsequent_fields_markers = [
        "_build_settings_redirect_url = build_settings_redirect_url_v1",
        "_require_menu_settings_owner_v1 = require_menu_settings_owner_v1",
        "@router.",
    ]
    ...
    if (
        "update_sidebar_menu_subsequent_fields" not in subsequent_fields_content
        and "subsequent" not in subsequent_fields_content.lower()
    ):
        raise RuntimeError(
            "O ficheiro subsequent_fields_handlers.py não parece conter lógica de Campos subsequentes."
        )
    ...
    missing_subsequent_fields = [
        marker
        for marker in required_subsequent_fields_markers
        if marker not in subsequent_fields_content
    ]
    ...
    if missing_subsequent_fields:
        raise RuntimeError(
            "Marcadores ausentes no subsequent_fields_handlers.py: "
            + ", ".join(missing_subsequent_fields)
        )

    duplicated_routes = []

    for route in re.findall(
        r"""@router\.(?:get|post|put|patch|delete)\(\s*["']([^"']*subsequent[^"']*)["']""",
        settings_content,
        flags=re.IGNORECASE,
    ):
    ...
    if duplicated_routes:
        raise RuntimeError(
            "Rotas de Campos subsequentes ainda ficaram duplicadas no settings_handlers.py: "
            + ", ".join(duplicated_routes)
        )

    print("OK: Campos subsequentes do Processo/Menu movidos para subsequent_fields_handlers.py.")
    print("OK: settings_handlers.py ficou apenas com import do subprocesso Campos subsequentes.")
    print("OK: rotas de Campos subsequentes permanecem registadas no router partilhado.")
