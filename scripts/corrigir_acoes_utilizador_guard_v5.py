...
####################################################################################
# (3.1) REESCREVER JS DE AÇÕES DO UTILIZADOR
####################################################################################
...
####################################################################################
# (3.2) REMOVER TABLE-ICON-BTN DO BOTÃO DELETE NO PARTIAL
####################################################################################
...
if 'class="admin-user-delete-button-v1"' in partial and "style=" not in partial:
    partial = partial.replace(
...
####################################################################################
# (3.3) ATUALIZAR CACHE BUSTER DO SCRIPT NO TEMPLATE PRINCIPAL
####################################################################################
...
if "admin_user_inactive_delete_button_v1.js" not in new_user:
    script_tag = '<script src="/static/js/modules/admin_user_inactive_delete_button_v1.js?v=20260513-user-actions-guard-v5"></script>'
    new_user = new_user.replace("{% endblock %}", script_tag + "\n{% endblock %}", 1)
else:
    new_user = re.sub(
        r'admin_user_inactive_delete_button_v1\.js\?v=[^"]+',
        'admin_user_inactive_delete_button_v1.js?v=20260513-user-actions-guard-v5',
        new_user,
    )
...
require_v1("APPVERBO_ADMIN_USER_ACTION_GUARD_V5" in js_content, "ERRO: guard V5 não foi criado.")
require_v1("navigateUserAction" in js_content, "ERRO: guard V5 não trata editar/exibir.")
require_v1("submitDeleteForm" in js_content, "ERRO: guard V5 não trata delete.")
require_v1("stopImmediatePropagation" in js_content, "ERRO: guard V5 não bloqueia listeners concorrentes.")
require_v1("HTMLFormElement.prototype.submit.call(form)" in js_content, "ERRO: delete não usa submissão nativa.")
require_v1("admin-user-delete-button-v1" in partial_content, "ERRO: partial sem botão delete.")
require_v1("table-icon-btn table-icon-btn-danger admin-user-delete-button-v1" not in partial_content, "ERRO: delete ainda tem classe table-icon-btn.")
require_v1("20260513-user-actions-guard-v5" in new_user_content, "ERRO: cache buster V5 não aplicado.")

print("OK: guard V5 das ações do utilizador aplicado.")
