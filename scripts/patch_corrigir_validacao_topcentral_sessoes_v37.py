...
TOP_CACHE = "/static/js/modules/top_menu_active_v1.js?v=20260505-topcentral-sessoes-v37"
SIDEBAR_CACHE = "/static/js/modules/sidebar_sections_layout_v1.js?v=20260505-topcentral-sessoes-v37"
ADMIN_CSS_CACHE = "/static/css/modules/admin_subprocesses_v1.css?v=20260505-topcentral-sessoes-v37"
ADMIN_JS_CACHE = "/static/js/modules/admin_subprocesses_v1.js?v=20260505-topcentral-sessoes-v37"

TEMPLATE_MARKER_START = "<!-- APPVERBO_TOPCENTRAL_ACIMA_SESSOES_V37_START -->"
TEMPLATE_MARKER_END = "<!-- APPVERBO_TOPCENTRAL_ACIMA_SESSOES_V37_END -->"
...
def fail_v37(message: str) -> None:
    print(f"ERRO: {message}")
    sys.exit(1)


def read_text_v37(path: Path) -> str:
    return path.read_text(encoding="utf-8-sig")


def write_text_v37(path: Path, content: str) -> None:
    path.write_text(content.rstrip() + "\n", encoding="utf-8")


def normalizar_texto_v37(valor: str) -> str:
    texto = str(valor or "")
...
def strip_tags_v37(html: str) -> str:
    return re.sub(r"<[^>]+>", " ", html)


def remove_marked_html_family_v37(content: str, family: str) -> str:
    pattern = re.compile(
...
def remove_marked_js_family_v37(content: str, family: str) -> str:
    pattern = re.compile(
...
def find_main_container_open_end_v37(content: str) -> int:
    ...
    if not main_match:
        fail_v37('não foi encontrado <main class="content"> em templates/new_user.html')
    ...
    if not container_match:
        fail_v37('não foi encontrado <div class="container"> dentro de <main class="content">')

    return main_match.end() + container_match.end()


def find_section_bounds_by_id_v37(content: str, section_id: str):
    ...
    fail_v37(f"section sem fechamento: {section_id}")


def find_menu_tabs_sections_v37(content: str):
    pattern = re.compile(
...
def remove_section_by_id_v37(content: str, section_id: str) -> str:
    bounds = find_section_bounds_by_id_v37(content, section_id)
...
def remove_sections_containing_v37(content: str, terms: list[str]) -> str:
    section_pattern = re.compile(r'\n?[ \t]*<section\b[\s\S]*?</section>[ \t]*\n?', re.I)

    def repl(match: re.Match) -> str:
        section_html = match.group(0)
        raw = section_html.lower()
        clean = normalizar_texto_v37(strip_tags_v37(section_html))

        for term in terms:
            term_raw = str(term or "").lower()
            term_clean = normalizar_texto_v37(term)

            if term_raw in raw or term_clean in clean:
                return "\n"
...
def remove_bare_admin_subprocess_if_v37(content: str) -> str:
    pattern = re.compile(
...
def remove_orphan_admin_subprocess_macro_v37(content: str) -> str:
    pattern = re.compile(
...
def ensure_macro_import_v37(content: str) -> str:
...
def ensure_head_asset_v37(content: str, asset_html: str, asset_name: str) -> str:
    content = re.sub(
    ...
    if not head_match:
        fail_v37(f"não encontrei bloco head_extra para inserir {asset_name}")

    insert_at = head_match.start(3)
    return content[:insert_at] + "  " + asset_html + "\n" + content[insert_at:]


def ensure_single_menu_tabs_first_v37(content: str) -> str:
    tabs_sections = find_menu_tabs_sections_v37(content)
    ...
    container_open_end = find_main_container_open_end_v37(without_tabs)
...
def insert_sessoes_after_tabs_v37(content: str) -> str:
    bounds = find_section_bounds_by_id_v37(content, "menu-tabs-card")

    if not bounds:
        fail_v37("menu-tabs-card ausente ao inserir subprocesso Sessões.")
...
for file_path in [TEMPLATE_PATH, TOP_MENU_JS_PATH, SIDEBAR_JS_PATH]:
    if not file_path.exists():
        fail_v37(f"ficheiro não encontrado: {file_path}")
...
template = read_text_v37(TEMPLATE_PATH)
...
for family in [
    ...
    template = remove_marked_html_family_v37(template, family)

template = remove_bare_admin_subprocess_if_v37(template)
template = remove_orphan_admin_subprocess_macro_v37(template)
...
for section_id in [
    ...
    template = remove_section_by_id_v37(template, section_id)

template = remove_sections_containing_v37(
    template,
    [
...
template = ensure_macro_import_v37(template)
template = ensure_single_menu_tabs_first_v37(template)
template = insert_sessoes_after_tabs_v37(template)

template = ensure_head_asset_v37(
    template,
...
write_text_v37(TEMPLATE_PATH, template)

print("OK: template corrigido: TopCentral antes de Sessões e sem bloco legado visual.")


####################################################################################
# (4) CORRIGIR TOP MENU: URL E ACTIVE
####################################################################################

top_js = read_text_v37(TOP_MENU_JS_PATH)
...
for family in [
    ...
    top_js = remove_marked_js_family_v37(top_js, family)
...
write_text_v37(TOP_MENU_JS_PATH, top_js)

print("OK: top_menu_active_v1.js validado para Menu e Sessões.")
...
sidebar_js = read_text_v37(SIDEBAR_JS_PATH)
...
for family in [
    ...
    sidebar_js = remove_marked_js_family_v37(sidebar_js, family)
...
write_text_v37(SIDEBAR_JS_PATH, sidebar_js)

print("OK: sidebar_sections_layout_v1.js protegido contra criação visual legada.")
...
for template_file in (ROOT / "templates").rglob("*.html"):
    content = read_text_v37(template_file)
    original = content
    ...
    if content != original:
        write_text_v37(template_file, content)
...
template_validado = read_text_v37(TEMPLATE_PATH)
top_validado = read_text_v37(TOP_MENU_JS_PATH)
sidebar_validado = read_text_v37(SIDEBAR_JS_PATH)

tabs = find_menu_tabs_sections_v37(template_validado)

if len(tabs) != 1:
    fail_v37(f"deve existir exatamente 1 menu-tabs-card; encontrado: {len(tabs)}")

if TEMPLATE_MARKER_START not in template_validado:
    fail_v37("marcador V37 ausente no template.")

tabs_bounds = find_section_bounds_by_id_v37(template_validado, "menu-tabs-card")
macro_index = template_validado.find(TEMPLATE_MARKER_START)

if not tabs_bounds:
    fail_v37("menu-tabs-card ausente no template.")

_, tabs_end = tabs_bounds

if macro_index < tabs_end:
    fail_v37("bloco de Sessões está antes ou dentro do menu-tabs-card.")

container_open_end = find_main_container_open_end_v37(template_validado)
...
if conteudo_entre_container_e_tabs:
    fail_v37("menu-tabs-card não está como primeiro elemento do container central.")

template_normalizado = normalizar_texto_v37(strip_tags_v37(template_validado))

for forbidden_text in [
    "sessoes do sidebar",
]:
    if forbidden_text in template_normalizado:
        fail_v37(f"texto legado ainda existe no template: {forbidden_text}")
...
for forbidden_template in [
    ...
    if forbidden_template in template_validado:
        fail_v37(f"conteúdo visual legado ainda existe no template: {forbidden_template}")

for forbidden_top in [
    "APPVERBO_SESSOES_ABAS_ACIMA",
    "APPVERBO_TOPCENTRAL_ANTES_CRIAR",
]:
    if forbidden_top in top_validado:
        fail_v37(f"corretor antigo ainda existe no top_menu_active_v1.js: {forbidden_top}")

if NEW_SESSOES_URL not in top_validado:
    fail_v37("URL correta de Sessões ausente em top_menu_active_v1.js.")

if 'menu: "/users/new?menu=administrativo&admin_tab=menu"' not in top_validado:
    fail_v37("URL correta de Menu ausente em top_menu_active_v1.js.")

if 'adminTab === "menu"' not in top_validado:
    fail_v37("active de admin_tab=menu ausente em top_menu_active_v1.js.")

if "function existeServerRenderSessoes_v32()" not in sidebar_validado:
    fail_v37("função existeServerRenderSessoes_v32 ausente no sidebar JS.")

if "APPVERBO_GUARD_OBTER_CARD_CRIACAO_SESSOES_V32" not in sidebar_validado:
    fail_v37("guard obter card V32 ausente no sidebar JS.")

if "APPVERBO_GUARD_MOVER_CARD_CRIACAO_SESSOES_V32" not in sidebar_validado:
    fail_v37("guard mover card V32 ausente no sidebar JS.")

for file_path in [TEMPLATE_PATH, TOP_MENU_JS_PATH, SIDEBAR_JS_PATH]:
    content = read_text_v37(file_path)

    if not content.endswith("\n"):
        fail_v37(f"ficheiro sem newline final: {file_path}")

    if content.endswith("\n\n"):
        fail_v37(f"ficheiro com linha em branco extra no EOF: {file_path}")

print("OK: validações V37 concluídas.")
