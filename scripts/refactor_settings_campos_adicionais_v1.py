...
ADDITIONAL_FIELDS_PATH = SETTINGS_PACKAGE_DIR / "additional_fields_handlers.py"
...
MOVED_MARKER_START = "# APPVERBO_SETTINGS_HANDLERS_ADDITIONAL_FIELDS_MOVED_V1_START"
MOVED_MARKER_END = "# APPVERBO_SETTINGS_HANDLERS_ADDITIONAL_FIELDS_MOVED_V1_END"
...
def should_move_additional_fields_block_v1(block: str) -> bool:
    ...
    if "update_sidebar_menu_additional_fields_v1" in block:
        return True

    if "update_sidebar_menu_additional_fields(" in block:
        return True

    if "move_sidebar_menu_additional_field" in block:
        return True

    if "additional_field" in block_lower and "process_" not in route_path:
        return True
...
def extract_selected_router_blocks_v1(content: str) -> tuple[list[str], str, list[str]]:
    ...
    for block_start, block_end in ranges:
        ...
        if should_move_additional_fields_block_v1(block):
...
####################################################################################
# (3) MÓDULO DE CAMPOS ADICIONAIS
####################################################################################

def build_additional_fields_module_v1(blocks: list[str]) -> str:
...
def patch_settings_handlers_v1() -> None:
    ...
    already_moved = "additional_fields_handlers_v1" in settings_content

    if already_moved:
        if not ADDITIONAL_FIELDS_PATH.exists():
            raise RuntimeError(
                "settings_handlers.py já importa additional_fields_handlers_v1, "
                "mas o ficheiro additional_fields_handlers.py não existe."
            )

        print("INFO: Campos adicionais já estavam importados.")
        return
    ...
    if not extracted_blocks:
        raise RuntimeError(
            "Nenhum endpoint de Campos adicionais foi encontrado para mover. "
            "Confirme se ainda existem rotas/handlers de additional fields em settings_handlers.py."
        )

    additional_fields_module = build_additional_fields_module_v1(extracted_blocks)
    write_text_v1(ADDITIONAL_FIELDS_PATH, additional_fields_module)
...
def validate_v1() -> None:
    settings_content = read_text_v1(SETTINGS_HANDLERS_PATH)
    additional_content = read_text_v1(ADDITIONAL_FIELDS_PATH)
    ...
    if (
        "update_sidebar_menu_additional_fields" not in additional_content
        and "move_sidebar_menu_additional_field" not in additional_content
        and "additional" not in additional_content.lower()
    ):
        raise RuntimeError(
            "O ficheiro additional_fields_handlers.py não parece conter lógica de Campos adicionais."
        )
    ...
    missing_additional = [
        marker
        for marker in required_additional_markers
        if marker not in additional_content
    ]
    ...
    if missing_additional:
        raise RuntimeError(
            "Marcadores ausentes no additional_fields_handlers.py: "
    ...
    for route in re.findall(
        r"""@router\.(?:get|post|put|patch|delete)\(\s*["']([^"']*additional[^"']*)["']""",
        settings_content,
        flags=re.IGNORECASE,
    ):
    ...
    if duplicated_routes:
        raise RuntimeError(
            "Rotas de Campos adicionais ainda ficaram duplicadas no settings_handlers.py: "
            + ", ".join(duplicated_routes)
        )

    print("OK: Campos adicionais do Menu movidos para additional_fields_handlers.py.")
    print("OK: settings_handlers.py ficou apenas com import do subprocesso Campos adicionais.")
    print("OK: rotas de Campos adicionais permanecem registadas no router partilhado.")
