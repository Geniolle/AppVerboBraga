...
def json_pretty_v2(value: Any) -> str:
...
def unique_keep_order_v2(values: list[str]) -> list[str]:
    result: list[str] = []
    seen_values: set[str] = set()

    for raw_value in values:
        clean_value = normalize_key_v2(raw_value)
...
####################################################################################
# (5) EXTRAIR CAMPOS DISPONÍVEIS V2
####################################################################################

def extract_available_fields_v2(menu_config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "visible_field_keys": unique_keep_order_v2(visible_field_keys),
        "header_by_field": header_by_field,
    }


####################################################################################
# (6) NORMALIZAR REGRAS DE QUALQUER FORMATO V2
####################################################################################

def normalize_rule_from_dict_v2(raw_rule: dict[str, Any], index: int, source_key: str) -> dict[str, str]:
    ...
    trigger_field = normalize_key_v2(
        ...
        or raw_rule.get("campo_acionador")
    )
    ...
    field_key = normalize_key_v2(
        ...
        or raw_rule.get("campo_subsequente")
    ...
    trigger_value = str(
        ...
        or raw_rule.get("valor_acionador")
        or ""
    ).strip()
...
def normalize_rule_from_string_v2(raw_rule: str, index: int, source_key: str) -> dict[str, str]:
    clean_value = str(raw_rule or "").strip()

    return {
        "source_key": source_key,
        "key": f"{source_key}_{index + 1}",
        "trigger_field": "",
        "field_key": normalize_key_v2(clean_value),
        "operator": "",
        "trigger_value": "",
    }


def normalize_rules_from_storage_v2(menu_config: dict[str, Any]) -> list[dict[str, str]]:
    normalized_rules: list[dict[str, str]] = []

    for source_key in SUBSEQUENT_KEYS_TO_CHECK:
        raw_value = menu_config.get(source_key)
        ...
        for index, raw_rule in enumerate(raw_value):
            if isinstance(raw_rule, dict):
                normalized_rules.append(
                    normalize_rule_from_dict_v2(raw_rule, index, source_key)
                )
                continue

            if isinstance(raw_rule, str):
                normalized_rules.append(
                    normalize_rule_from_string_v2(raw_rule, index, source_key)
                )
                continue
...
####################################################################################
# (7) VALIDAR E IMPRIMIR RESULTADO V2
####################################################################################
...
def main_v2() -> None:
    with SessionLocal() as session:
        ...
        available_fields = extract_available_fields_v2(menu_config)
        normalized_rules = normalize_rules_from_storage_v2(menu_config)
        ...
        found_any_storage = False

        for source_key in SUBSEQUENT_KEYS_TO_CHECK:
            if source_key not in menu_config:
                continue

            found_any_storage = True
            raw_value = menu_config.get(source_key)
            value_type = type(raw_value).__name__
            value_count = len(raw_value) if isinstance(raw_value, list) else "-"

            print("")
            print(f"- {source_key}: tipo={value_type} | quantidade={value_count}")
            print("  Conteúdo bruto:")
            print(json_pretty_v2(raw_value))

        if not found_any_storage:
        ...
        if normalized_rules:
            print("EXISTE configuração criada/armazenada em Campos Subsequentes.")
            print(f"Quantidade total encontrada: {len(normalized_rules)}")
        else:
            print("NÃO EXISTE configuração criada/armazenada em Campos Subsequentes.")
            print("Quantidade total encontrada: 0")

        print("")
        print("===== ITENS NORMALIZADOS ENCONTRADOS =====")

        if not normalized_rules:
            print("- Nenhum item encontrado.")

        for index, rule in enumerate(normalized_rules, start=1):
            trigger_label = available_fields["labels_by_key"].get(
                rule["trigger_field"],
                rule["trigger_field"],
            )
            field_label = available_fields["labels_by_key"].get(
                rule["field_key"],
                rule["field_key"],
            )
            ...
            print("")
            print(f"Item #{index}")
            print(f"- origem: {format_value_v2(rule['source_key'])}")
            print(f"- key: {format_value_v2(rule['key'])}")
            print(f"- campo acionador: {format_value_v2(rule['trigger_field'])} - {format_value_v2(trigger_label)}")
            print(f"- condição: {format_value_v2(rule['operator'])} - {get_operator_label_v2(rule['operator'])}")
            print(f"- valor acionador: {format_value_v2(rule['trigger_value'])}")
            print(f"- campo subsequente: {format_value_v2(rule['field_key'])} - {format_value_v2(field_label)}")

            issues: list[str] = []

            if rule["source_key"] == "subsequent_fields" and not rule["trigger_field"]:
                issues.append(
                    "este item está em subsequent_fields, mas não tem campo acionador; "
                    "pode ser lista legada/campo alvo, não regra completa"
                )

            if rule["field_key"] and rule["field_key"] not in available_fields["visible_field_keys"]:
                issues.append(
                    f"campo subsequente não está na lista atual de campos visíveis: {rule['field_key']}"
                )
            ...
            if issues:
                ...
                print("- observações: OK")

        print("")
        print("===== CAMPOS VISÍVEIS DISPONÍVEIS NO MEU PERFIL =====")
        ...
        print("")
        print("===== RESUMO =====")
        print(f"Existe algum armazenamento de campos subsequentes: {'SIM' if found_any_storage else 'NÃO'}")
        print(f"Quantidade total normalizada: {len(normalized_rules)}")
