...
NEW_JS_PATH = ROOT / "static" / "js" / "modules" / "process_fields_config_manager_v3.js"
...
####################################################################################
# (3) LIMPAR BLOCO BACKEND INJETADO ANTERIORMENTE, SE EXISTIR
####################################################################################
...
selected_block_pattern = re.compile(
    r"\n[ \t]*" + re.escape(selected_marker_start) + r"[\s\S]*?" + re.escape(selected_marker_end) + r"\n",
    re.S,
)

if selected_marker_start in settings_handlers:
    settings_handlers = selected_block_pattern.sub("\n", settings_handlers, count=1)
    print("OK: bloco backend temporário V6 removido.")
else:
    print("OK: bloco backend temporário V6 não existia.")
...
try:
    ...
    fail(f"settings_handlers.py ficaria com erro de sintaxe após limpeza: {exc}")


####################################################################################
# (4) CRIAR JS V3 DO GESTOR OFICIAL
####################################################################################
...
print("OK: process_fields_config_manager_v3.js criado.")


####################################################################################
# (5) ATUALIZAR TEMPLATE PARA USAR SOMENTE O GESTOR OFICIAL V3
####################################################################################
...
template = re.sub(
    ...
    '/static/js/modules/process_fields_config_manager_v3.js?v=20260430-fields-config-v3',
    template,
)

template = re.sub(
    r'/static/js/modules/process_fields_config_manager_v2\.js\?v=[^"]+',
    '/static/js/modules/process_fields_config_manager_v3.js?v=20260430-fields-config-v3',
    template,
)

if "process_fields_config_manager_v3.js" not in template:
    script_tag = '<script src="/static/js/modules/process_fields_config_manager_v3.js?v=20260430-fields-config-v3"></script>'
...
print("OK: settings_handlers.py limpo.")
print("OK: new_user.html atualizado para carregar process_fields_config_manager_v3.js.")
print("OK: patch_process_fields_config_manager_v3 concluído.")
