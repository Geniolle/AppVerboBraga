...
TOP_CACHE = "/static/js/modules/top_menu_active_v1.js?v=20260505-remover-lixo-sessoes-sidebar-v35"
SIDEBAR_CACHE = "/static/js/modules/sidebar_sections_layout_v1.js?v=20260505-remover-lixo-sessoes-sidebar-v35"
...
def fail_v35(message: str) -> None:
    print(f"ERRO: {message}")
    sys.exit(1)


def read_text_v35(path: Path) -> str:
    return path.read_text(encoding="utf-8-sig")


def write_text_v35(path: Path, content: str) -> None:
    path.write_text(content.rstrip() + "\n", encoding="utf-8")


def normalizar_texto_v35(valor: str) -> str:
    texto = str(valor or "")
...
def strip_tags_v35(html: str) -> str:
    return re.sub(r"<[^>]+>", " ", html)


def find_menu_tabs_sections_v35(content: str):
    pattern = re.compile(
...
def find_main_container_open_end_v35(content: str) -> int:
    ...
    if not main_match:
        fail_v35('não foi encontrado <main class="content"> em templates/new_user.html')
    ...
    if not container_match:
        fail_v35('não foi encontrado <div class="container"> dentro de <main class="content">')

    return main_match.end() + container_match.end()


def garantir_topcentral_primeiro_card_v35(template_html: str) -> str:
    tabs_sections = find_menu_tabs_sections_v35(template_html)
    ...
    container_open_end = find_main_container_open_end_v35(template_without_tabs)
...
for file_path in [TEMPLATE_PATH, SIDEBAR_JS_PATH, TOP_MENU_JS_PATH]:
    if not file_path.exists():
        fail_v35(f"ficheiro não encontrado: {file_path}")
...
template_html = read_text_v35(TEMPLATE_PATH)
removed_count = 0
...
def remove_marker_block_if_legacy_v35(match: re.Match) -> str:
    global removed_count
    bloco = match.group(0)
    texto = normalizar_texto_v35(strip_tags_v35(bloco))
...
template_html = marker_pattern.sub(remove_marker_block_if_legacy_v35, template_html)

section_pattern = re.compile(r'\n?[ \t]*<section\b[\s\S]*?</section>[ \t]*\n?', re.I)

def section_is_legacy_sessoes_v35(section_html: str) -> bool:
    raw = section_html.lower()
    texto = normalizar_texto_v35(strip_tags_v35(section_html))
    ...
    return has_legacy_title or (has_legacy_id and has_legacy_class_or_action)

def remove_legacy_section_v35(match: re.Match) -> str:
    global removed_count
    section_html = match.group(0)

    if section_is_legacy_sessoes_v35(section_html):
        removed_count += 1
        return "\n"

    return section_html

template_html = section_pattern.sub(remove_legacy_section_v35, template_html)
template_html = garantir_topcentral_primeiro_card_v35(template_html)

write_text_v35(TEMPLATE_PATH, template_html)

print(f"OK: blocos legados de Sessões removidos do template: {removed_count}")


####################################################################################
# (4) REFORCAR GUARDA NO SIDEBAR LEGADO COM SUBSTITUICAO SEGURA
####################################################################################

sidebar_js = read_text_v35(SIDEBAR_JS_PATH)
sidebar_js = sidebar_js.replace(OLD_SESSOES_URL, NEW_SESSOES_URL)

new_guard = '''function existeServerRenderSessoes_v32() {
...
write_text_v35(SIDEBAR_JS_PATH, sidebar_js)
...
top_menu_js = read_text_v35(TOP_MENU_JS_PATH)
...
write_text_v35(TOP_MENU_JS_PATH, top_menu_js)
...
for template_file in (ROOT / "templates").rglob("*.html"):
    content = read_text_v35(template_file)
    original = content
    ...
    if content != original:
        write_text_v35(template_file, content)
...
template_validado = read_text_v35(TEMPLATE_PATH)
sidebar_validado = read_text_v35(SIDEBAR_JS_PATH)
top_validado = read_text_v35(TOP_MENU_JS_PATH)

template_normalizado = normalizar_texto_v35(strip_tags_v35(template_validado))

if "sessoes do sidebar" in template_normalizado:
    fail_v35("texto legado 'Sessões do sidebar' ainda existe no template.")

tabs_sections = find_menu_tabs_sections_v35(template_validado)

if len(tabs_sections) != 1:
    fail_v35(f"deve existir exatamente 1 menu-tabs-card; encontrado: {len(tabs_sections)}")

container_open_end = find_main_container_open_end_v35(template_validado)
...
if conteudo_entre_container_e_tabs:
    fail_v35("menu-tabs-card não está como primeiro elemento do container central.")

if NEW_SESSOES_URL not in top_validado:
    fail_v35("URL correta de Sessões ausente em top_menu_active_v1.js.")

if "function existeServerRenderSessoes_v32()" not in sidebar_validado:
    fail_v35("função existeServerRenderSessoes_v32 ausente em sidebar_sections_layout_v1.js.")

if "APPVERBO_GUARD_OBTER_CARD_CRIACAO_SESSOES_V32" not in sidebar_validado:
    fail_v35("guard obter card de criação V32 ausente em sidebar_sections_layout_v1.js.")

if "APPVERBO_GUARD_MOVER_CARD_CRIACAO_SESSOES_V32" not in sidebar_validado:
    fail_v35("guard mover card de criação V32 ausente em sidebar_sections_layout_v1.js.")

for file_path in [TEMPLATE_PATH, SIDEBAR_JS_PATH, TOP_MENU_JS_PATH]:
    content = read_text_v35(file_path)
    if not content.endswith("\n"):
        fail_v35(f"ficheiro sem newline final: {file_path}")
    if content.endswith("\n\n"):
        fail_v35(f"ficheiro com linha em branco extra no EOF: {file_path}")

print("OK: validações V35 concluídas.")
