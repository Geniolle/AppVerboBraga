...
def backup_file_v1(path: Path, suffix: str) -> Path:
    if not path.exists():
        raise FileNotFoundError(f"Ficheiro não encontrado para backup: {path}")
...
def replace_once_v1(content: str, old: str, new: str, description: str) -> str:
    if old not in content:
        raise RuntimeError(f"Não encontrei o bloco esperado para: {description}")
...
def validate_v2_base_exists_v1() -> None:
    ...
    if missing:
        raise RuntimeError(
            "Motor Admin Subprocess V2 ainda não existe. Execute primeiro o patch do motor V2. "
...
def patch_v2_registry_v1() -> None:
    ...
    if 'edit_param="entity_id"' in content:
        content = content.replace('edit_param="entity_id"', 'edit_param="entity_edit_id"', 1)

    if 'edit_param="entity_edit_id"' not in content:
        raise RuntimeError("Não foi possível configurar edit_param='entity_edit_id' na Entidade V2.")
...
def patch_v2_service_v1() -> None:
    ...
    old_block = '''def default_admin_subprocess_return_url_v2(config: AdminSubprocessConfigV2) -> str:
    return (
...
'''
    ...
    old_block_alt = '''def default_admin_subprocess_return_url_v2(config: AdminSubprocessConfigV2) -> str:
    return (
        f"/users/new?menu=administrativo&admin_tab={config.key}"
        f"&target=%23{config.resolved_default_target}"
        f"#${config.resolved_default_target}"
...
'''
    ...
    if old_block in content:
        ...
        content = content.replace(old_block_alt, new_block, 1)
    elif "def default_admin_subprocess_return_url_v2" in content and "target=%23{config.resolved_default_target}" in content:
        content = re.sub(
        ...
        raise RuntimeError("Não encontrei default_admin_subprocess_return_url_v2 para ajustar.")
...
def patch_v2_handlers_v1() -> None:
    ...
    entity_edit_id: str = "",
        ...
        content = replace_once_v1(
            ...
            old_signature,
            new_signature,
            "assinatura standalone V2 com entity_edit_id",
        )

    if 'edit_key=edit_key,' in content and 'edit_key=edit_key or entity_edit_id,' not in content:
...
def patch_v2_macro_v1() -> None:
    ...
    elif "{{ state.config.edit_param }}={{ row_key }}" not in content:
        raise RuntimeError("Não encontrei o href de edição no macro V2.")

    write_text_v1(V2_MACRO_PATH, content)


####################################################################################
# (8) PATCH PAGE_HANDLER
####################################################################################
...
'''
    ...
    if anchor not in content:
        raise RuntimeError("Não encontrei import build_admin_subprocess_state em page_handler.py.")

    return content.replace(anchor, anchor + import_block, 1)


def patch_page_handler_initial_target_v1(content: str) -> str:
    old_block = '''    if clean_menu_key == "administrativo":
    ...
    new_block = '''    if clean_menu_key == "administrativo":
        if settings_edit_key:
            return "#settings-menu-edit-card", ""
        if resolved_admin_tab == "contas":
            return "#admin-account-status-card", ""
    ...
    raise RuntimeError("Não encontrei bloco administrativo em _resolve_initial_menu_target.")


def patch_page_handler_state_v1(content: str) -> str:
    marker_start = "# APPVERBO_ADMIN_SUBPROCESS_STATE_ENTIDADE_V2_START"
    marker_end = "# APPVERBO_ADMIN_SUBPROCESS_STATE_ENTIDADE_V2_END"

    new_block = '''    # APPVERBO_ADMIN_SUBPROCESS_STATE_ENTIDADE_V2_START
    if resolved_admin_tab == "entidade":
...
'''
    ...
    if marker_start in content and marker_end in content:
        ...
        return pattern.sub(lambda _match: new_block, content)
...
'''
    ...
    if anchor not in content:
        raise RuntimeError("Não encontrei bloco de admin_subprocess_state_v2 para Sessões.")

    return content.replace(anchor, new_block + anchor, 1)
...
def patch_page_handler_v1() -> None:
    ...
    content = patch_page_handler_initial_target_v1(content)
...
def patch_template_imports_v1(content: str) -> str:
    ...
    if anchor not in content:
        raise RuntimeError("Não encontrei import de macro admin_subprocess.html.")

    return content.replace(anchor, anchor + import_line + "\n", 1)


def patch_template_assets_v1(content: str) -> str:
    css_line = '<link rel="stylesheet" href="/static/css/modules/admin_subprocesses_v2.css?v=20260506-entidade-v2-users-new-v1">'
    js_line = '<script src="/static/js/modules/admin_subprocesses_v2.js?v=20260506-entidade-v2-users-new-v1" defer></script>'
    integration_line = '<script src="/static/js/modules/admin_subprocesses_v2_integration.js?v=20260506-entidade-v2-users-new-v1" defer></script>'

    if css_line not in content:
        anchor = '<link rel="stylesheet" href="/static/css/modules/admin_subprocesses_v1.css'
        index = content.find(anchor)

        if index == -1:
            raise RuntimeError("Não encontrei CSS admin_subprocesses_v1 no template.")

        line_start = content.rfind("\n", 0, index) + 1
        content = content[:line_start] + css_line + "\n" + content[line_start:]

    if js_line not in content:
        anchor = '<script src="/static/js/modules/admin_subprocesses_v1.js'
        index = content.find(anchor)

        if index == -1:
            raise RuntimeError("Não encontrei JS admin_subprocesses_v1 no template.")
        ...
        content = content[:line_end + 1] + "  " + js_line + "\n" + "  " + integration_line + "\n" + content[line_end + 1:]

    return content


def patch_template_render_block_v1(content: str) -> str:
    marker_start = "<!-- APPVERBO_ADMIN_ENTIDADE_V2_USERS_NEW_START -->"
    marker_end = "<!-- APPVERBO_ADMIN_ENTIDADE_V2_USERS_NEW_END -->"
...
'''
    ...
    anchor = "<!-- APPVERBO_CORRIGIR_ORDEM_ABAS_SESSOES_ADMIN_SUBPROCESS_V5_START -->"

    if anchor not in content:
        raise RuntimeError("Não encontrei bloco de renderização de Sessões no template.")
...
def patch_template_v1() -> None:
    ...
    required = [
        ...
        "admin_subprocesses_v2.css?v=20260506-entidade-v2-users-new-v1",
        "admin_subprocesses_v2_integration.js?v=20260506-entidade-v2-users-new-v1",
    ]
...
(function setupAdminEntidadeV2UsersNewV1() {
  "use strict";
  ...
  //###################################################################################
  // (2) OCULTAR LEGADO DA ENTIDADE
  //###################################################################################

  function hideLegacyEntidadeCardsV1() {
    if (!hasEntidadeV2RootV1()) {
      return;
    }
    ...
    legacySelectors.forEach((selector) => {
      document.querySelectorAll(selector).forEach(hideElementV1);
    });
  ...
  //###################################################################################
  // (3) AJUSTAR TARGET DA ABA ENTIDADE
  //###################################################################################

  function ensureEntidadeV2TargetV1() {
    if (!hasEntidadeV2RootV1()) {
      return;
    }
    ...
    try {
      ...
      // Ignora ambientes sem URLSearchParams/histórico.
    }
  }

  function runEntidadeV2IntegrationV1() {
    hideLegacyEntidadeCardsV1();
    ensureEntidadeV2TargetV1();
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", runEntidadeV2IntegrationV1);
  } else {
    runEntidadeV2IntegrationV1();
  }

  window.setTimeout(runEntidadeV2IntegrationV1, 100);
  window.setTimeout(runEntidadeV2IntegrationV1, 400);
  window.setTimeout(runEntidadeV2IntegrationV1, 1200);
})();
// APPVERBO_ADMIN_ENTIDADE_V2_USERS_NEW_END
'''
...
def validate_v1() -> None:
    ...
    checks = [
        ...
        ("template assets V2", "admin_subprocesses_v2_integration.js?v=20260506-entidade-v2-users-new-v1" in template_content),
        ...
        ("service return url sem hash", "target=%23{config.resolved_default_target}" in service_content),
        ("integration hide legacy", "hideLegacyEntidadeCardsV1" in integration_js_content),
    ]
    ...
    print("OK: legado da Entidade é ocultado quando V2 estiver ativo.")
    print("OK: edição usa entity_edit_id e permanece em /users/new.")
...
def main() -> None:
    ...
    for path in files_to_backup:
        backup_path = backup_file_v1(path, "integrar_entidade_v2_users_new")
    ...
    print("OK: integração Entidade V2 concluída.")
