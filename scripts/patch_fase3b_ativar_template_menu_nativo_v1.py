...
{# (1) TEMPLATE DO SUBPROCESSO ADMINISTRATIVO MENU - V2 #}
{# ################################################################################### #}
...
{#
  ...
  Modo controlado:
  - admin_menu_template_mode=native ativa a renderização nativa.
  - Sem a flag, o template fica preparado, mas invisível.
#}

{# ################################################################################### #}
{# (3) RENDERIZAÇÃO NATIVA CONTROLADA #}
{# ################################################################################### #}

{% set is_admin_menu_native_v1 = admin_menu_template_mode|default("", true) == "native" %}
...
{% if admin_menu_state %}
  <div
    ...
    data-admin-template-mode="{{ admin_menu_template_mode|default('', true) }}"
...
def patch_menu_template_v2() -> None:
...
{# Fase 3B: include ativo em modo controlado por admin_menu_template_mode=native. #}
{% set appverbo_current_admin_tab_v1 = resolved_admin_tab|default(admin_tab|default("", true), true) %}
{% if admin_menu_template_ready_v1 and admin_menu_template_mode == "native" and appverbo_current_admin_tab_v1 == "menu" %}
...
def patch_page_handler_context_v1() -> None:
    ...
    if '"admin_menu_template_mode": str(request.query_params.get("admin_menu_template_mode") or "").strip().lower(),' in text:
        return
    ...
    replacement = (
        anchor
        + '        "admin_menu_template_mode": str(request.query_params.get("admin_menu_template_mode") or "").strip().lower(),\n'
    )
...
def validate_files_v1() -> None:
    ...
    required_menu_template = [
        "TEMPLATE DO SUBPROCESSO ADMINISTRATIVO MENU - V2",
        "is_admin_menu_native_v1",
        'admin_menu_template_mode|default("", true) == "native"',
        'id="admin-menu-template-v1"',
        "render_admin_subprocess_state(admin_menu_state)",
        "hideLegacyMenuCardsV1",
    ...
    required_new_user = [
        ...
        'admin_menu_template_mode == "native"',
...
def main() -> None:
    patch_menu_template_v2()
    patch_new_user_include_v1()
    patch_page_handler_context_v1()
    validate_files_v1()
    print("OK: Fase 3B aplicada. Template do Menu ativo em modo controlado native.")
