...
####################################################################################
# (3.1) GARANTIR JS V6 COM LISTENER EM CAPTURA
####################################################################################
...
####################################################################################
# (3.2) MOVER SCRIPT PARA O INÍCIO DO BLOCO DE SCRIPTS
####################################################################################

new_user_path = "templates/new_user.html"
new_user = read_text_v1(new_user_path)

script_tag = '<script src="/static/js/modules/admin_user_inactive_delete_button_v1.js?v=20260513-user-actions-guard-v6"></script>'

new_user = re.sub(
    r'\n?\s*<script src="/static/js/modules/admin_user_inactive_delete_button_v1\.js\?v=[^"]+"></script>',
    "",
    new_user,
)

block_match = re.search(r'{%\s*block\s+scripts\s*%}', new_user)

if block_match:
    insert_at = block_match.end()
    new_user = new_user[:insert_at] + "\n" + script_tag + new_user[insert_at:]
else:
    first_script = re.search(r'<script\b', new_user)
    if first_script:
        insert_at = first_script.start()
        new_user = new_user[:insert_at] + script_tag + "\n" + new_user[insert_at:]
    else:
        new_user = new_user + "\n" + script_tag + "\n"
...
####################################################################################
# (3.3) REMOVER LISTENER ANTIGO V3 DO PARTIAL REUTILIZÁVEL
####################################################################################
...
partial = re.sub(
    r'\n<script>\s*\(function\s*\(\)\s*\{[\s\S]*?APPVERBO_DELETE_USER_CLICK_V3[\s\S]*?</script>',
    "",
    partial,
    count=1,
)
...
if 'class="admin-user-delete-button-v1"' in partial and 'style="border:0;background:transparent;cursor:pointer;padding:4px 6px;color:inherit;"' not in partial:
    partial = partial.replace(
        'class="admin-user-delete-button-v1"\n              title="Eliminar utilizador"',
        'class="admin-user-delete-button-v1"\n              style="border:0;background:transparent;cursor:pointer;padding:4px 6px;color:inherit;"\n              title="Eliminar utilizador"'
    )
...
new_user_content = read_text_v1("templates/new_user.html")
partial_content = read_text_v1("templates/partials/admin_user_table_v1.html")

require_v1("APPVERBO_ADMIN_USER_ACTION_GUARD_V6" in js_content, "ERRO: guard V6 não foi criado.")
require_v1("navigateUserAction" in js_content, "ERRO: V6 não trata editar/exibir.")
require_v1("submitDeleteForm" in js_content, "ERRO: V6 não trata delete.")
require_v1("stopImmediatePropagation" in js_content, "ERRO: V6 não bloqueia listeners concorrentes.")
require_v1("20260513-user-actions-guard-v6" in new_user_content, "ERRO: new_user.html não usa cache buster V6.")
require_v1(new_user_content.count("admin_user_inactive_delete_button_v1.js") == 1, "ERRO: script guard duplicado no new_user.html.")
require_v1("APPVERBO_DELETE_USER_CLICK_V3" not in partial_content, "ERRO: listener antigo V3 ainda existe no partial.")
require_v1("table-icon-btn table-icon-btn-danger admin-user-delete-button-v1" not in partial_content, "ERRO: delete ainda usa classe de ação antiga.")

print("OK: guard V6 movido para o início e conflito V3 removido.")
