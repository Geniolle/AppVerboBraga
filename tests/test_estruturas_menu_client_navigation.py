...
def test_client_navigation_to_estruturas_menu_shows_tables_without_refresh() -> None:
    ...
    try:
        ...
        assert state["rowCount"] > 0, "Tabela Menus ativos sem linhas apos navegacao por clique"

        active_submenu = [
            e.text.strip() for e in driver.find_elements(By.CSS_SELECTOR, ".submenu-item.active")
        ]
...
def test_administrativo_clean_url_tab_still_activates_correct_tab() -> None:
    ...
    try:
        ...
        active_submenu = [
            e.text.strip().lower() for e in driver.find_elements(By.CSS_SELECTOR, ".submenu-item.active")
        ]
...
def test_top_menu_active_scopes_sticky_label_to_administrativo() -> None:
    js_text = (PROJECT_ROOT / "static" / "js" / "modules" / "top_menu_active_v1.js").read_text(encoding="utf-8")
...
def test_client_navigation_to_estruturas_menu_shows_inactive_card_without_refresh() -> None:
    driver = _build_driver_v1()
    wait = WebDriverWait(driver, 30)
    ...
    try:
        ...
        wait.until(lambda drv: _card_state(drv, "menu-subprocess-card-inactive").get("computedDisplay") == "block")

        state = _card_state(driver, "menu-subprocess-card-inactive")
        assert state["exists"], "Card 'Menus inativos' nao existe no DOM apos navegacao por clique"
        assert state["computedDisplay"] == "block", (
