...
def test_dynamic_process_action_card_is_separate_from_content_card() -> None:
    ...
    toolbar_index = html_text.index('class="appverbo-process-action-toolbar-v1"')
    ...
    assert "appverbo-process-action-card-v1" in action_card_snippet

    toggle_snippet = html_text[toolbar_index:toggle_button_index + 200]
    assert "appverbo-process-action-toggle-v1" in toggle_snippet
...
def test_admin_subprocess_create_toggle_uses_same_css_classes() -> None:
    ...
    assert "appverbo-process-action-toolbar-v1" in macro_text
    assert "appverbo-process-action-toggle-v1" in macro_text
    assert "appverbo-process-action-card-v1" in macro_text
