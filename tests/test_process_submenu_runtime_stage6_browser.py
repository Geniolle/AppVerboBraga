...
####################################################################################
# (2) STAGE 6 SUBMENU VALIDATION
####################################################################################


def test_stage6_process_submenu_runtime_clicks_keep_runtime_behavior() -> None:
    driver = _build_driver_v1()
    wait = WebDriverWait(driver, 30)

    try:
        _login_admin_v1(driver, wait)
        driver.get("http://127.0.0.1:8000/users/new?menu=administrativo&admin_tab=entidade")
        ...
        assert driver.execute_script("return Boolean(window.AppGenesisTopSubmenu);")
        ...
        wait.until(lambda drv: drv.execute_script("return document.readyState") == "complete")
        meu_perfil_submenus = driver.execute_script(
            """
            return Array.from(document.querySelectorAll(".submenu-item"))
              .map((element) => String(element.textContent || "").trim())
              .filter(Boolean);
            """
        )
        if "Dados de morada" in meu_perfil_submenus:
            _click_submenu_v1(driver, "Dados de morada")
            wait.until(lambda drv: "Dados de morada" in _active_submenu_labels_v1(drv))
            wait.until(lambda drv: _card_is_visible_v1(drv, "perfil-morada-card"))
        else:
            wait.until(lambda drv: _card_is_visible_v1(drv, "perfil-pessoal-card"))
        ...
        for menu_key in candidate_menu_keys:
            ...
            wait.until(lambda drv: len(drv.find_elements(By.CSS_SELECTOR, ".submenu-item")) >= 1)
            dynamic_labels = driver.execute_script(
                """
                return Array.from(document.querySelectorAll(".submenu-item[data-dynamic-process-section]"))
                  .map((element) => String(element.textContent || "").trim())
                  .filter(Boolean);
                """
            )
            if not dynamic_labels:
                continue
            _click_submenu_v1(driver, dynamic_labels[0])
            wait.until(lambda drv: dynamic_labels[0] in _active_submenu_labels_v1(drv))
            wait.until(lambda drv: _card_is_visible_v1(drv, "dynamic-process-card"))
            matched_menu_key = menu_key
            break

        assert matched_menu_key, "Nenhum processo dinâmico navegável pelo submenu foi encontrado."
