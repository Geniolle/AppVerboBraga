from pathlib import Path

from tests.test_new_user_runtime_functional_v1 import (
    _build_chrome_driver_v1,
    _inject_js_file_v1,
    _load_blank_page_v1,
)


PROJECT_ROOT = Path(__file__).resolve().parents[1]


####################################################################################
# (1) O RUNTIME DE QUANTIDADE DEVE EXPOR A API CANONICA EM EXECUCAO
####################################################################################


def test_process_quantity_runtime_v1_exposes_expected_api() -> None:
    driver = _build_chrome_driver_v1()
    try:
        _load_blank_page_v1(driver, "<html><body></body></html>")
        _inject_js_file_v1(driver, "static/js/modules/profile_field_registry_v1.js")
        _inject_js_file_v1(driver, "static/js/modules/process_quantity_runtime_v1.js")

        api = driver.execute_script(
            """
            const runtime = window.AppGenesisProcessQuantityRuntimeV1 || {};
            return {
              initialize: typeof runtime.initialize,
              render: typeof runtime.render,
              sync: typeof runtime.sync,
              getValues: typeof runtime.getValues,
              destroy: typeof runtime.destroy,
              createMeuPerfilQuantityAdapterV1: typeof runtime.createMeuPerfilQuantityAdapterV1,
              createDynamicProcessQuantityAdapterV1: typeof runtime.createDynamicProcessQuantityAdapterV1
            };
            """
        )

        assert api == {
            "initialize": "function",
            "render": "function",
            "sync": "function",
            "getValues": "function",
            "destroy": "function",
            "createMeuPerfilQuantityAdapterV1": "function",
            "createDynamicProcessQuantityAdapterV1": "function",
        }
    finally:
        driver.quit()


####################################################################################
# (2) O RUNTIME DE QUANTIDADE DEVE SUPORTAR LISTA DINAMICA NO MEU PERFIL
####################################################################################


def test_process_quantity_runtime_v1_supports_dynamic_list_mode_for_profile_agregados() -> None:
    driver = _build_chrome_driver_v1()
    try:
        _load_blank_page_v1(
            driver,
            """
            <html>
              <body>
                <section id="perfil-pessoal-card">
                  <div class="profile-readonly">
                    <div class="personal-grid"></div>
                  </div>
                  <form class="profile-edit-form" method="post" action="/users/profile/personal">
                    <div class="personal-grid">
                      <div class="field" data-profile-field-key="custom_quantos_filhos_tens">
                        <label for="custom_field__custom_quantos_filhos_tens">Quantidade</label>
                        <input id="custom_field__custom_quantos_filhos_tens" name="custom_field__custom_quantos_filhos_tens" type="number" value="2">
                      </div>
                    </div>
                  </form>
                </section>
              </body>
            </html>
            """,
        )
        _inject_js_file_v1(driver, "static/js/modules/profile_field_registry_v1.js")
        _inject_js_file_v1(driver, "static/js/modules/process_quantity_runtime_v1.js")

        driver.execute_script(
            """
            const setting = {
              process_quantity_fields: [
                {
                  key: "qty_agregados",
                  quantity_field_key: "custom_quantos_filhos_tens",
                  repeated_field_keys: ["custom_nome_do_agregado", "custom_data_nascimento_do_agregado"],
                  header_key: "custom_dados_de_agregados",
                  max_items: 3,
                  item_label: "Agregado",
                  label: "Agregados",
                  interaction_mode: "dynamic_list"
                }
              ],
              process_field_options: [
                { key: "custom_quantos_filhos_tens", label: "Quantidade", field_type: "number" },
                { key: "custom_nome_do_agregado", label: "Nome do agregado", field_type: "text" },
                { key: "custom_data_nascimento_do_agregado", label: "Data de nascimento do agregado", field_type: "date" }
              ]
            };
            window.__quantityContextV1 = {
              mode: "profile",
              adapterName: "profile",
              root: document,
              formEl: document.querySelector(".profile-edit-form"),
              readonlyGridEl: document.querySelector(".profile-readonly .personal-grid"),
              editGridEl: document.querySelector(".profile-edit-form .personal-grid"),
              setting,
              rules: setting.process_quantity_fields,
              valuesByRule: {
                qty_agregados: [
                  { custom_nome_do_agregado: "Ana", custom_data_nascimento_do_agregado: "01/01/2000" },
                  { custom_nome_do_agregado: "Bruno", custom_data_nascimento_do_agregado: "02/02/2002" }
                ]
              },
              getSetting: () => setting,
              getCurrentSection: () => "custom_dados_de_agregados"
            };
            window.AppGenesisProcessQuantityRuntimeV1.initialize(window.__quantityContextV1);
            """
        )

        initial_state = driver.execute_script(
            """
            const hidden = document.querySelector("input[name='process_quantity_payload__qty_agregados']");
            const quantityControl = document.querySelector("[name='custom_field__custom_quantos_filhos_tens']");
            return {
              itemCount: document.querySelectorAll("[data-process-quantity-rule-key='qty_agregados'] .profile-quantity-dynamic-item-edit-v1").length,
              hiddenValue: hidden ? hidden.value : null,
              quantityHidden: quantityControl ? (quantityControl.closest('.field') || quantityControl.parentElement || {}).hidden === true : false,
              quantityValue: quantityControl ? quantityControl.value : null,
              addDisabled: document.querySelector("[data-process-quantity-add-item='1']") ? document.querySelector("[data-process-quantity-add-item='1']").disabled : null
            };
            """
        )

        assert initial_state == {
            "itemCount": 2,
            "hiddenValue": '[{"custom_nome_do_agregado":"Ana","custom_data_nascimento_do_agregado":"01/01/2000"},{"custom_nome_do_agregado":"Bruno","custom_data_nascimento_do_agregado":"02/02/2002"}]',
            "quantityHidden": True,
            "quantityValue": "2",
            "addDisabled": False,
        }

        driver.execute_script(
            """
            document.querySelector("[data-process-quantity-add-item='1']").click();
            """
        )

        after_add = driver.execute_script(
            """
            const hidden = document.querySelector("input[name='process_quantity_payload__qty_agregados']");
            const quantityControl = document.querySelector("[name='custom_field__custom_quantos_filhos_tens']");
            return {
              itemCount: document.querySelectorAll("[data-process-quantity-rule-key='qty_agregados'] .profile-quantity-dynamic-item-edit-v1").length,
              hiddenValue: hidden ? hidden.value : null,
              quantityValue: quantityControl ? quantityControl.value : null
            };
            """
        )

        assert after_add["itemCount"] == 3
        assert after_add["quantityValue"] == "3"
        assert after_add["hiddenValue"] == '[{"custom_nome_do_agregado":"Ana","custom_data_nascimento_do_agregado":"01/01/2000"},{"custom_nome_do_agregado":"Bruno","custom_data_nascimento_do_agregado":"02/02/2002"},{}]'

        driver.execute_script(
            """
            const buttons = document.querySelectorAll("[data-process-quantity-remove-item='1']");
            buttons[buttons.length - 1].click();
            """
        )

        after_remove = driver.execute_script(
            """
            const hidden = document.querySelector("input[name='process_quantity_payload__qty_agregados']");
            const quantityControl = document.querySelector("[name='custom_field__custom_quantos_filhos_tens']");
            return {
              itemCount: document.querySelectorAll("[data-process-quantity-rule-key='qty_agregados'] .profile-quantity-dynamic-item-edit-v1").length,
              hiddenValue: hidden ? hidden.value : null,
              quantityValue: quantityControl ? quantityControl.value : null
            };
            """
        )

        assert after_remove["itemCount"] == 2
        assert after_remove["quantityValue"] == "2"
        assert after_remove["hiddenValue"] == '[{"custom_nome_do_agregado":"Ana","custom_data_nascimento_do_agregado":"01/01/2000"},{"custom_nome_do_agregado":"Bruno","custom_data_nascimento_do_agregado":"02/02/2002"}]'
    finally:
        driver.quit()


####################################################################################
# (2) A LISTA DINAMICA DEVE PRESERVAR IDS E NORMALIZAR DATAS LEGADAS
####################################################################################


def test_process_quantity_runtime_v1_preserves_item_ids_and_normalizes_legacy_dates() -> None:
    driver = _build_chrome_driver_v1()
    try:
        _load_blank_page_v1(
            driver,
            """
            <html>
              <body>
                <section id="perfil-pessoal-card">
                  <div class="profile-readonly">
                    <div class="personal-grid"></div>
                  </div>
                  <form class="profile-edit-form" method="post" action="/users/profile/personal">
                    <div class="personal-grid">
                      <div class="field" data-profile-field-key="custom_quantos_filhos_tens">
                        <label for="custom_field__custom_quantos_filhos_tens">Quantidade</label>
                        <input id="custom_field__custom_quantos_filhos_tens" name="custom_field__custom_quantos_filhos_tens" type="number" value="3">
                      </div>
                    </div>
                  </form>
                </section>
              </body>
            </html>
            """,
        )
        _inject_js_file_v1(driver, "static/js/modules/profile_field_registry_v1.js")
        _inject_js_file_v1(driver, "static/js/modules/process_quantity_runtime_v1.js")

        driver.execute_script(
            """
            const setting = {
              process_quantity_fields: [
                {
                  key: "qty_agregados",
                  quantity_field_key: "custom_quantos_filhos_tens",
                  repeated_field_keys: ["custom_nome_do_agregado", "custom_data_nascimento_do_agregado"],
                  header_key: "custom_dados_de_agregados",
                  max_items: 5,
                  item_label: "Agregado",
                  label: "Agregados",
                  interaction_mode: "dynamic_list"
                }
              ],
              process_field_options: [
                { key: "custom_quantos_filhos_tens", label: "Quantidade", field_type: "number" },
                { key: "custom_nome_do_agregado", label: "Nome do agregado", field_type: "text" },
                { key: "custom_data_nascimento_do_agregado", label: "Data de nascimento do agregado", field_type: "date" }
              ]
            };
            window.__quantityContextV1 = {
              mode: "profile",
              adapterName: "profile",
              root: document,
              formEl: document.querySelector(".profile-edit-form"),
              readonlyGridEl: document.querySelector(".profile-readonly .personal-grid"),
              editGridEl: document.querySelector(".profile-edit-form .personal-grid"),
              setting,
              rules: setting.process_quantity_fields,
              valuesByRule: {
                qty_agregados: [
                  {
                    item_id: "item-a",
                    custom_nome_do_agregado: "Ana",
                    custom_data_nascimento_do_agregado: "01012000"
                  },
                  {
                    item_id: "item-b",
                    custom_nome_do_agregado: "Bruno",
                    custom_data_nascimento_do_agregado: "02/02/2002"
                  },
                  {
                    item_id: "item-c",
                    custom_nome_do_agregado: "Carla",
                    custom_data_nascimento_do_agregado: "2003-03-03"
                  }
                ]
              },
              getSetting: () => setting,
              getCurrentSection: () => "custom_dados_de_agregados"
            };
            window.AppGenesisProcessQuantityRuntimeV1.initialize(window.__quantityContextV1);
            """
        )

        initial_state = driver.execute_script(
            """
            const hidden = document.querySelector("input[name='process_quantity_payload__qty_agregados']");
            const dateValues = Array.from(document.querySelectorAll("input[type='date']")).map((input) => input.value);
            const itemIds = Array.from(document.querySelectorAll("[data-process-quantity-rule-key='qty_agregados'] input[type='hidden'][data-process-quantity-field-key='item_id']")).map((input) => input.value);
            return {
              itemCount: document.querySelectorAll("[data-process-quantity-rule-key='qty_agregados'] .profile-quantity-dynamic-item-edit-v1").length,
              hiddenValue: hidden ? hidden.value : null,
              dateValues,
              itemIds
            };
            """
        )

        assert initial_state["itemCount"] == 3
        assert initial_state["dateValues"] == ["2000-01-01", "2002-02-02", "2003-03-03"]
        assert initial_state["itemIds"] == ["item-a", "item-b", "item-c"]
        assert initial_state["hiddenValue"] == (
            '[{"item_id":"item-a","custom_nome_do_agregado":"Ana","custom_data_nascimento_do_agregado":"01012000"},'
            '{"item_id":"item-b","custom_nome_do_agregado":"Bruno","custom_data_nascimento_do_agregado":"02/02/2002"},'
            '{"item_id":"item-c","custom_nome_do_agregado":"Carla","custom_data_nascimento_do_agregado":"2003-03-03"}]'
        )

        driver.execute_script(
            """
            const buttons = document.querySelectorAll("[data-process-quantity-remove-item='1']");
            buttons[1].click();
            """
        )

        after_remove = driver.execute_script(
            """
            const hidden = document.querySelector("input[name='process_quantity_payload__qty_agregados']");
            const itemIds = Array.from(document.querySelectorAll("[data-process-quantity-rule-key='qty_agregados'] input[type='hidden'][data-process-quantity-field-key='item_id']")).map((input) => input.value);
            return {
              itemCount: document.querySelectorAll("[data-process-quantity-rule-key='qty_agregados'] .profile-quantity-dynamic-item-edit-v1").length,
              hiddenValue: hidden ? hidden.value : null,
              itemIds
            };
            """
        )

        assert after_remove["itemCount"] == 2
        assert after_remove["itemIds"] == ["item-a", "item-c"]
        assert after_remove["hiddenValue"] == (
            '[{"item_id":"item-a","custom_nome_do_agregado":"Ana","custom_data_nascimento_do_agregado":"01012000"},'
            '{"item_id":"item-c","custom_nome_do_agregado":"Carla","custom_data_nascimento_do_agregado":"2003-03-03"}]'
        )
    finally:
        driver.quit()


####################################################################################
# (2) A REGRA dynamic_list DEVE RESPEITAR A SECTION ATIVA NO MEU PERFIL
####################################################################################


def test_process_quantity_runtime_v1_dynamic_list_resolves_profile_section_and_toggles_without_duplicates() -> None:
    driver = _build_chrome_driver_v1()
    try:
        _load_blank_page_v1(
            driver,
            """
            <html>
              <body>
                <section id="perfil-pessoal-card">
                  <div class="profile-readonly">
                    <div class="personal-grid"></div>
                  </div>
                  <input data-meu-perfil-section-input value="custom_dados_pessoais">
                  <form class="profile-edit-form" method="post" action="/users/profile/personal">
                    <div class="personal-grid">
                      <div class="field" data-profile-field-key="tem_filhos">
                        <label for="tem_filhos">Tem filhos</label>
                        <input id="tem_filhos" name="custom_field__tem_filhos" type="checkbox">
                      </div>
                      <div class="field" data-profile-field-key="custom_quantos_filhos_tens">
                        <label for="custom_field__custom_quantos_filhos_tens">Quantidade</label>
                        <input id="custom_field__custom_quantos_filhos_tens" name="custom_field__custom_quantos_filhos_tens" type="number" value="2">
                      </div>
                    </div>
                  </form>
                </section>
              </body>
            </html>
            """,
        )
        _inject_js_file_v1(driver, "static/js/modules/profile_field_registry_v1.js")
        _inject_js_file_v1(driver, "static/js/modules/process_quantity_runtime_v1.js")

        driver.execute_script(
            """
            const setting = {
              process_visible_field_rows: [
                { field_key: "tem_filhos", header_key: "custom_dados_pessoais" },
                { field_key: "custom_nome_do_agregado", header_key: "custom_dados_de_agregados" },
                { field_key: "custom_data_nascimento_do_agregado", header_key: "custom_dados_de_agregados" },
                { field_key: "custom_quantos_filhos_tens", header_key: "custom_dados_pessoais" }
              ],
              process_quantity_fields: [
                {
                  key: "qty_agregados",
                  quantity_field_key: "custom_quantos_filhos_tens",
                  repeated_field_keys: ["custom_nome_do_agregado", "custom_data_nascimento_do_agregado"],
                  max_items: 3,
                  item_label: "Agregado",
                  label: "Agregados",
                  interaction_mode: "dynamic_list"
                }
              ],
              process_field_options: [
                { key: "tem_filhos", label: "Tem filhos", field_type: "flag" },
                { key: "custom_quantos_filhos_tens", label: "Quantidade", field_type: "number" },
                { key: "custom_nome_do_agregado", label: "Nome do agregado", field_type: "text" },
                { key: "custom_data_nascimento_do_agregado", label: "Data de nascimento do agregado", field_type: "date" }
              ]
            };
            window.__quantityContextV1 = {
              mode: "profile",
              adapterName: "profile",
              root: document,
              formEl: document.querySelector(".profile-edit-form"),
              readonlyGridEl: document.querySelector(".profile-readonly .personal-grid"),
              editGridEl: document.querySelector(".profile-edit-form .personal-grid"),
              setting,
              rules: setting.process_quantity_fields,
              valuesByRule: {
                qty_agregados: [
                  { custom_nome_do_agregado: "Ana", custom_data_nascimento_do_agregado: "01/01/2000" },
                  { custom_nome_do_agregado: "Bruno", custom_data_nascimento_do_agregado: "02/02/2002" }
                ]
              },
              getSetting: () => setting,
              getCurrentSection: () => document.querySelector("[data-meu-perfil-section-input]").value
            };
            window.AppGenesisProcessQuantityRuntimeV1.initialize(window.__quantityContextV1);
            """
        )

        initial_state = driver.execute_script(
            """
            const hiddenPayloads = document.querySelectorAll("input[data-meu-perfil-quantity-payload='1']");
            return {
              agregadosVisible: document.querySelectorAll("[data-process-quantity-rule-key='qty_agregados'] .profile-quantity-dynamic-item-edit-v1").length,
              temFilhosVisible: document.querySelector("[data-profile-field-key='tem_filhos']") ? true : false,
              hiddenPayloadCount: hiddenPayloads.length
            };
            """
        )

        assert initial_state["agregadosVisible"] == 0
        assert initial_state["temFilhosVisible"] is True

        driver.execute_script(
            """
            document.querySelector("[data-meu-perfil-section-input]").value = "custom_dados_de_agregados";
            window.AppGenesisProcessQuantityRuntimeV1.render(window.__quantityContextV1);
            window.AppGenesisProcessQuantityRuntimeV1.sync(window.__quantityContextV1);
            """
        )

        agregados_state = driver.execute_script(
            """
            const payload = document.querySelector("input[name='process_quantity_payload__qty_agregados']");
            return {
              agregadosVisible: document.querySelectorAll("[data-process-quantity-rule-key='qty_agregados'] .profile-quantity-dynamic-item-edit-v1").length,
              itemIds: Array.from(document.querySelectorAll("[data-process-quantity-rule-key='qty_agregados'] input[type='hidden'][data-process-quantity-field-key='item_id']")).map((input) => input.value),
              payloadCount: document.querySelectorAll("input[data-meu-perfil-quantity-payload='1']").length,
              payloadValue: payload ? payload.value : null
            };
            """
        )

        assert agregados_state["agregadosVisible"] == 2
        assert len(agregados_state["itemIds"]) == 2
        assert agregados_state["itemIds"][0] != agregados_state["itemIds"][1]
        assert agregados_state["payloadCount"] == 1
        assert agregados_state["payloadValue"] is not None

        driver.execute_script(
            """
            document.querySelector("[data-meu-perfil-section-input]").value = "custom_dados_pessoais";
            window.AppGenesisProcessQuantityRuntimeV1.render(window.__quantityContextV1);
            window.AppGenesisProcessQuantityRuntimeV1.sync(window.__quantityContextV1);
            """
        )

        back_state = driver.execute_script(
            """
            return {
              agregadosVisible: document.querySelectorAll("[data-process-quantity-rule-key='qty_agregados'] .profile-quantity-dynamic-item-edit-v1").length,
              payloadCount: document.querySelectorAll("input[data-meu-perfil-quantity-payload='1']").length
            };
            """
        )

        assert back_state["agregadosVisible"] == 0
        assert back_state["payloadCount"] == 1
    finally:
        driver.quit()


####################################################################################
# (2) O RUNTIME DE QUANTIDADE DEVE USAR DELEGACAO E SUPORTAR MULTIPLAS REGRAS
####################################################################################


def test_process_quantity_runtime_v1_delegates_events_and_handles_multiple_rules_v1() -> None:
    driver = _build_chrome_driver_v1()
    try:
        _load_blank_page_v1(
            driver,
            """
            <html>
              <body>
                <section id="perfil-pessoal-card">
                  <div class="profile-readonly">
                    <div class="personal-grid"></div>
                  </div>
                  <form class="profile-edit-form" method="post" action="/users/profile/personal">
                    <div class="personal-grid">
                      <div class="field">
                        <label for="quantidade_a">Quantidade A</label>
                        <input id="quantidade_a" name="quantidade_a" type="number" value="1">
                      </div>
                      <div class="field">
                        <label for="quantidade_b">Quantidade B</label>
                        <input id="quantidade_b" name="quantidade_b" type="number" value="1">
                      </div>
                    </div>
                  </form>
                </section>
              </body>
            </html>
            """,
        )
        driver.execute_script(
            """
            window.AppGenesisProfileFieldRegistryV1 = {
              resolveControlName(fieldKey) {
                return String(fieldKey || "");
              },
              collectProfileValues() {
                return {};
              },
              getCurrentProfileSection() {
                return "dados";
              }
            };
            """
        )
        _inject_js_file_v1(driver, "static/js/modules/process_quantity_runtime_v1.js")

        driver.execute_script(
            """
            const setting = {
              process_quantity_fields: [
                {
                  key: "rule_a",
                  quantity_field_key: "quantidade_a",
                  repeated_field_keys: ["telefone_a"],
                  header_key: "dados",
                  max_items: 3,
                  item_label: "Contacto A"
                },
                {
                  key: "rule_b",
                  quantity_field_key: "quantidade_b",
                  repeated_field_keys: ["telefone_b"],
                  header_key: "dados",
                  max_items: 3,
                  item_label: "Contacto B"
                }
              ],
              process_field_options: [
                { key: "quantidade_a", label: "Quantidade A", field_type: "number" },
                { key: "quantidade_b", label: "Quantidade B", field_type: "number" },
                { key: "telefone_a", label: "Telefone A", field_type: "text" },
                { key: "telefone_b", label: "Telefone B", field_type: "text" }
              ]
            };
            const form = document.querySelector(".profile-edit-form");
            const readonlyGridEl = document.querySelector(".profile-readonly .personal-grid");
            const editGridEl = document.querySelector(".profile-edit-form .personal-grid");
            window.__quantityContextV1 = {
              mode: "profile",
              adapterName: "profile",
              root: document,
              formEl: form,
              readonlyGridEl,
              editGridEl,
              setting,
              rules: setting.process_quantity_fields,
              valuesByRule: {
                rule_a: [{ telefone_a: "111" }],
                rule_b: [{ telefone_b: "222" }]
              },
              getSetting: () => setting,
              getValues() {
                return {
                  rule_a: [{ telefone_a: "111" }],
                  rule_b: [{ telefone_b: "222" }]
                };
              },
              getCurrentSection: () => "dados"
            };
            window.AppGenesisProcessQuantityRuntimeV1.initialize(window.__quantityContextV1);
            """
        )

        initial_state = driver.execute_script(
            """
            const state = window.__quantityContextV1.__appGenesisProcessQuantityRuntimeV1 || {};
            const hiddenA = document.querySelector("input[name='process_quantity_payload__rule_a']");
            const hiddenB = document.querySelector("input[name='process_quantity_payload__rule_b']");
            return {
              listenerCount: Array.isArray(state.listeners) ? state.listeners.length : -1,
              ruleACount: document.querySelectorAll("[data-process-quantity-rule-key='rule_a'] .profile-quantity-item-v1").length,
              ruleBCount: document.querySelectorAll("[data-process-quantity-rule-key='rule_b'] .profile-quantity-item-v1").length,
              hiddenA: hiddenA ? hiddenA.value : null,
              hiddenB: hiddenB ? hiddenB.value : null
            };
            """
        )

        assert initial_state["listenerCount"] == 3
        assert initial_state["ruleACount"] == 1
        assert initial_state["ruleBCount"] == 1
        assert initial_state["hiddenA"] == '[{"telefone_a":"111"}]'
        assert initial_state["hiddenB"] == '[{"telefone_b":"222"}]'

        driver.execute_script(
            """
            const hidden = document.querySelector("input[name='process_quantity_payload__rule_a']");
            let writes = 0;
            let stored = hidden.value;
            Object.defineProperty(hidden, "value", {
              configurable: true,
              get() {
                return stored;
              },
              set(next) {
                writes += 1;
                stored = String(next);
              }
            });
            window.__hiddenWritesRuleA = () => writes;
            const control = document.getElementById("quantidade_a");
            control.value = "2";
            control.dispatchEvent(new Event("input", { bubbles: true }));
            """
        )

        after_single_event = driver.execute_script(
            """
            return {
              ruleACount: document.querySelectorAll("[data-process-quantity-rule-key='rule_a'] .profile-quantity-item-v1").length,
              hiddenWritesRuleA: window.__hiddenWritesRuleA()
            };
            """
        )

        assert after_single_event["hiddenWritesRuleA"] == 1
        assert after_single_event["ruleACount"] == 2

        driver.execute_script(
            """
            const control = document.getElementById("quantidade_b");
            control.value = "2";
            control.dispatchEvent(new Event("input", { bubbles: true }));
            """
        )

        after_multiple_rules = driver.execute_script(
            """
            return {
              ruleBCount: document.querySelectorAll("[data-process-quantity-rule-key='rule_b'] .profile-quantity-item-v1").length,
            };
            """
        )

        assert after_multiple_rules["ruleBCount"] == 2
    finally:
        driver.quit()


####################################################################################
# (2) O TEMPLATE DEVE CARREGAR O RUNTIME DE QUANTIDADE ANTES DO NEW_USER.JS
####################################################################################

def test_new_user_template_loads_quantity_runtime_before_new_user() -> None:
    template_text = (PROJECT_ROOT / "templates" / "new_user.html").read_text(encoding="utf-8")

    runtime_index = template_text.index('src="/static/js/modules/process_quantity_runtime_v1.js?v=20260722-dynamic-aggregates-v1"')
    new_user_index = template_text.index('src="/static/js/new_user.js')

    assert runtime_index < new_user_index
