...
(function registerSubmenuOverflowMenuV1() {
  ...
  const SUBMENU_SELECTOR_V1 = "#submenu-items";
  const MENU_CARD_SELECTOR_V1 = "#menu-tabs-card";
  ...
  function isTabItemV1(element) {
    ...
    return element.matches(".submenu-item, a, button, [data-admin-tab]");
  }
  ...
  function ensureOverflowStructureV1(itemsEl) {
    const cardEl = itemsEl.closest(MENU_CARD_SELECTOR_V1);
    if (!cardEl) {
      return null;
    }
  ...
  function setupSubmenuOverflowMenuV1() {
    const itemsEl = document.querySelector(SUBMENU_SELECTOR_V1);
    if (!itemsEl) {
      debugLogV1("setup.not_found", {});
      return null;
    }

    let state = itemsEl.__appverboSubmenuOverflowStateV1;
    if (!state) {
      state = ensureOverflowStructureV1(itemsEl);
      if (!state) {
        return null;
      }
      itemsEl.__appverboSubmenuOverflowStateV1 = state;
    }
