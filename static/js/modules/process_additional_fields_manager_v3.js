...
(function (window, document) {
  ...
  const SUPPORTED_TYPES = new Set(["text", "number", "email", "phone", "date", "flag", "header", "list", "link"]);
  const TEXTUAL_TYPES = new Set(["text", "number", "email", "phone", "link"]);
  ...
  function readEditorItem_v3(context) {
    ...
    const explicitKeyValue = toSafeString_v3(
      getInputValue_v3(editor, "[data-additional-field-editor-key]")
    ).trim();
    const rawKey = explicitKeyValue || label;
    ...
    return {
      ...
      label,
      fieldType,
      autoGeneratedKey: explicitKeyValue === "",
  ...
  function resolveUniqueItemKey_v3(item, context) {
    if (!item || typeof item !== "object") {
      return item;
    }

    const editingId = toSafeString_v3(context && context.state ? context.state.editingId : "");
    const items = Array.isArray(context && context.items) ? context.items : [];
    const reservedKeys = new Set();

    items.forEach((existingItem) => {
      const sameId = toSafeString_v3(existingItem && existingItem.__managerId) === editingId;
      if (sameId) {
        return;
      }
      const existingKey = normalizeFieldKey_v3(existingItem && existingItem.key);
      if (existingKey) {
        reservedKeys.add(normalizeLookup_v3(existingKey));
      }
    });

    const currentKey = normalizeFieldKey_v3(item.key);
    if (!currentKey) {
      return item;
    }

    const currentLookup = normalizeLookup_v3(currentKey);
    if (!reservedKeys.has(currentLookup)) {
      item.key = currentKey;
      item.id = currentKey;
      return item;
    }

    if (!item.autoGeneratedKey) {
      return item;
    }

    const labelBaseKey = normalizeFieldKey_v3(item.label || currentKey);
    const typeSuffix = item.fieldType === "header" ? "_header" : "_field";
    const typedBaseKey = normalizeFieldKey_v3(`${labelBaseKey}${typeSuffix}`) || currentKey;
    let candidateKey = typedBaseKey;
    let suffixIndex = 2;

    while (reservedKeys.has(normalizeLookup_v3(candidateKey))) {
      candidateKey = `${typedBaseKey}_${suffixIndex}`;
      suffixIndex += 1;
    }
