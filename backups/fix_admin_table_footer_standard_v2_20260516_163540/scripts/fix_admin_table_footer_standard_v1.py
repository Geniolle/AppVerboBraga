...
def insert_after_once_v1(content: str, marker: str, insertion: str, label: str) -> str:
    ...
    if index < 0:
        raise RuntimeError(f"ERRO: marcador nao encontrado para {label}: {marker}")

    insert_position = index + len(marker)
    return content[:insert_position] + "\n" + insertion + content[insert_position:]
...
####################################################################################
# (5) CORRIGIR PARTIAL DE UTILIZADORES
####################################################################################
...
def patch_admin_user_table_partial_v1() -> None:
    ...
    if footer_call not in content:
        first_else_index = content.find("{% else %}")

        require_v1(
            first_else_index >= 0,
            "ERRO: nao foi encontrado {% else %} no partial de utilizadores.",
        )

        table_close_index = content.rfind("</table>", 0, first_else_index)

        require_v1(
            table_close_index >= 0,
            "ERRO: nao foi encontrado </table> antes do {% else %} no partial de utilizadores.",
        )

        table_close_end = table_close_index + len("</table>")

        before = content[:table_close_end]
        after = content[first_else_index:]
...
def patch_admin_subprocess_macro_v1() -> None:
    ...
    if footer_call not in content:
        ...
        if marker in content:
            ...
            table_close_index = content.find("</table>")

            require_v1(
                table_close_index >= 0,
                "ERRO: nao foi encontrado </table> no macro de subprocessos.",
            )

            wrap_close_index = content.find("</div>", table_close_index)

            require_v1(
                wrap_close_index >= 0,
                "ERRO: nao foi encontrado </div> depois da tabela no macro de subprocessos.",
            )

            wrap_close_end = wrap_close_index + len("</div>")
            content = content[:wrap_close_end] + "\n      " + footer_call + content[wrap_close_end:]
...
def validate_content_v1() -> None:
    files = {
        "footer_partial": read_text_v1("templates/partials/admin_table_footer_standard_v1.html"),
        "footer_css": read_text_v1("static/css/modules/admin_table_footer_standard_v1.css"),
        "footer_js": read_text_v1("static/js/modules/admin_table_footer_standard_v1.js"),
    ...
    checks = {
        "macro reutilizavel": "macro render_admin_table_footer_standard_v1" in files["footer_partial"],
        "data attr footer": "data-admin-table-footer-standard-v1" in files["footer_partial"],
        "css marker": "APPVERBO_ADMIN_TABLE_FOOTER_STANDARD_V1_START" in files["footer_css"],
        "js api": "window.AppVerboAdminTableFooterStandard_v1" in files["footer_js"],
        "users import": 'from "partials/admin_table_footer_standard_v1.html" import render_admin_table_footer_standard_v1' in files["users_partial"],
        "users footer call": "render_admin_table_footer_standard_v1(table_id=table_id" in files["users_partial"],
        "subprocess import": 'from "partials/admin_table_footer_standard_v1.html" import render_admin_table_footer_standard_v1' in files["subprocess_macro"],
        "subprocess table id": 'id="admin-subprocess-{{ state.config.key }}-{{ status_value }}-table"' in files["subprocess_macro"],
        "subprocess footer call": 'table_id="admin-subprocess-" ~ state.config.key ~ "-" ~ status_value ~ "-table"' in files["subprocess_macro"],
        "template css": "admin_table_footer_standard_v1.css" in files["template"],
        "template js": "admin_table_footer_standard_v1.js" in files["template"],
    }
...
def main_v1() -> None:
    ...
    print("OK: correcao robusta aplicada.")
    print("OK: rodape padrao reutilizavel ativo.")
    print("OK: partial de utilizadores corrigido.")
    print("OK: macro de subprocessos corrigido.")
    print("OK: template principal corrigido.")
