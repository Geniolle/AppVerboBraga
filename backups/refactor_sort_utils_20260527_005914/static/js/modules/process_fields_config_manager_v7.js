...
(function (window, document) {
  ...
  function compararTextoOrdenacao_v7(a, b) {
  ...
  function ordenarItensPorCabecalho_v7(items, state) {
    ...
    const sorted = source.sort(function (leftItem, rightItem) {
      const leftHeader = obterHeaderLabelOrdenacao_v7(leftItem, state);
      const rightHeader = obterHeaderLabelOrdenacao_v7(rightItem, state);

      const headerCompare = compararTextoOrdenacao_v7(leftHeader, rightHeader);
      if (headerCompare !== 0) {
        return headerCompare;
      }

      const leftOrder = Number.isFinite(Number(leftItem && leftItem.__managerOrder))
        ? Number(leftItem.__managerOrder)
        : Number.MAX_SAFE_INTEGER;
      const rightOrder = Number.isFinite(Number(rightItem && rightItem.__managerOrder))
        ? Number(rightItem.__managerOrder)
        : Number.MAX_SAFE_INTEGER;
      ...
      return compararTextoOrdenacao_v7(
    ...
    return sorted.map(function (item, index) {
      const headerLabel = obterHeaderLabelOrdenacao_v7(item, state);
