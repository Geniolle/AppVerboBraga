...
####################################################################################
# (2) CSS ISOLADO - IGUALAR AO LEGADO COM CARD SEPARADO
####################################################################################
...
.admin-user-shadow-footer-v1 .pagination button {
  background: #ffffff;
  color: #7c8ba3;
  border: 1px solid #d9e3f2;
}

.admin-user-shadow-footer-v1 .pagination button:disabled {
  background: #ffffff;
  color: #c4ccda;
  border-color: #e5ebf5;
  opacity: 1;
...
.admin-user-shadow-footer-v1 .pagination button[data-admin-user-shadow-current-page] {
  background: #1f5fbf;
  color: #ffffff;
  border-color: #1f5fbf;
  opacity: 1;
...
.admin-user-shadow-inactive-card-v1 h3 {
  margin-top: 0;
}
...
####################################################################################
# (3) PARTIAL - SEPARAR ATIVOS E INATIVOS EM CARDS DIFERENTES
####################################################################################

partial_path = ROOT / "templates" / "partials" / "admin_user_shadow_readonly_v1.html"

partial_content = '''\
{% macro render_admin_user_shadow_cell_v6(row, column) %}
...
{% macro render_admin_user_shadow_table_v6(table_key, rows, columns) %}
<div
    ...
    <table class="admin-subprocess-table-v1 admin-user-shadow-table-v1">
      ...
      <tbody>
        ...
        <tr data-admin-user-shadow-row="{{ row.get('id', '') }}">
          {% for column in columns %}
          <td>{{ render_admin_user_shadow_cell_v6(row, column) }}</td>
          {% endfor %}
          <td>{{ render_admin_user_shadow_actions_v5(row) }}</td>
        </tr>
        {% endfor %}
      </tbody>
    </table>
...
<link rel="stylesheet" href="/static/css/modules/admin_user_shadow_table_v1.css?v=20260512-utilizador-shadow-layout-v6">
...
>
  ...
  <div class="admin-subsection">
    ...
    {% if state.active_rows %}
      {{ render_admin_user_shadow_table_v6("active", state.active_rows, state.config.columns) }}
    {% else %}
...
>
  ...
  {% if state.inactive_rows %}
    {{ render_admin_user_shadow_table_v6("inactive", state.inactive_rows, state.config.columns) }}
  {% else %}
    <p class="empty">Sem utilizadores inativos, pendentes ou bloqueados no processo nativo.</p>
  {% endif %}

  <script src="/static/js/modules/admin_user_shadow_table_v1.js?v=20260512-utilizador-shadow-table-v3" defer></script>
</section>
{% endmacro %}
...
####################################################################################
# (4) JS - GARANTIR QUE SETAS DESABILITADAS NÃO FICAM COMO ATIVAS
####################################################################################
...
####################################################################################
# (5) ATUALIZAR NEW_USER.HTML PARA MACRO V6
####################################################################################
...
if "render_admin_user_shadow_readonly_v6(admin_subprocess_shadow_state)" not in template_content:
    raise RuntimeError("Render V6 do Utilizador shadow não encontrado no new_user.html")
...
print("OK: Utilizadores inativos agora ficam em card separado.")
print("OK: select de entradas por página aumentado.")
print("OK: paginação ajustada para deixar azul apenas o número ativo.")
