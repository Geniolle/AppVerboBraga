...
// APPVERBO_UTILIZADOR_EXIBIR_TOGGLE_V4_START
(function () {
  "use strict";

  if (window.__appverboUtilizadorExibirToggleV4Loaded) {
    return;
  }

  window.__appverboUtilizadorExibirToggleV4Loaded = true;

  const VIEW_PARAM = "user_view";
  const EDIT_ID_PARAM = "user_edit_id";
  const PANEL_MARKER = "data-appverbo-user-view-panel-v4";

  function normalize_v4(value) {
    return String(value || "")
      .replace(/\s+/g, " ")
      .trim()
      .toLowerCase();
  ...
  function getParams_v4() {
    return new URLSearchParams(window.location.search || "");
  }

  function isUtilizadorPage_v4() {
    const params = getParams_v4();
    const path = normalize_v4(window.location.pathname);
  ...
  function hasUserViewTitle_v4(element) {
    const text = textOf_v4(element);
  ...
  function hasReadonlyUserFields_v4(element) {
    const text = textOf_v4(element);
  ...
  function findPanelFromTitle_v4(titleElement) {
    if (!titleElement) {
      return null;
    }
    ...
    for (let level = 0; level < 12 && current && current !== document.body; level += 1) {
      if (hasUserViewTitle_v4(current) && hasReadonlyUserFields_v4(current)) {
  ...
  function closePanel_v4(event) {
    if (event) {
      event.preventDefault();
      event.stopPropagation();
    }

    window.location.href = buildClosedUrl_v4();
  }

  function bindCloseButtons_v4(panel) {
    if (!panel || panel.dataset.appverboUserViewCloseBoundV4 === "1") {
      return;
    }
    ...
    controls.forEach(function (control) {
      const text = textOf_v4(control);

      if (text === "fechar" || text.indexOf("fechar") >= 0) {
        control.addEventListener("click", closePanel_v4);
      }
    });

    panel.dataset.appverboUserViewCloseBoundV4 = "1";
  }

  function applyUserViewVisibility_v4() {
    if (!isUtilizadorPage_v4()) {
      return;
    }

    const titleElement = findTitleElement_v4();

    if (!titleElement) {
      return;
    }

    const panel = findPanelFromTitle_v4(titleElement);
    ...
    if (isViewMode_v4()) {
      ...
      panel.dataset.appverboUserViewOpenV4 = "1";
      bindCloseButtons_v4(panel);
      return;
    }

    panel.hidden = true;
    panel.style.display = "none";
    panel.dataset.appverboUserViewOpenV4 = "0";
  }

  function scheduleApply_v4() {
    window.clearTimeout(window.__appverboUtilizadorExibirToggleTimerV4);
    window.__appverboUtilizadorExibirToggleTimerV4 = window.setTimeout(applyUserViewVisibility_v4, 80);
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", applyUserViewVisibility_v4);
  }
  else {
    applyUserViewVisibility_v4();
  }

  window.addEventListener("load", applyUserViewVisibility_v4);
  window.addEventListener("popstate", applyUserViewVisibility_v4);

  if (document.body && window.MutationObserver) {
    const observer = new MutationObserver(scheduleApply_v4);
...
// APPVERBO_UTILIZADOR_EXIBIR_TOGGLE_V4_END
