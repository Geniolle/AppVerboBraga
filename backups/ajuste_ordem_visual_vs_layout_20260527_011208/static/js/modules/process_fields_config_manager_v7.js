...
(function (window, document) {
  ...
  //###################################################################################
  // (6A) ORDENACAO POR CABECALHO
  //###################################################################################

  function obterSortUtilsV7() {
    const sortUtils = window.AppVerboConfigurableItemsSortUtils;
    if (!sortUtils || typeof sortUtils !== "object") {
      return null;
    }
  ...
  function obterHeaderLabelOrdenacao_v7(item, state) {
    const explicitLabel = limparLabelCabecalho_v7(item && item.headerLabel ? item.headerLabel : "");
    const mappedLabel = limparLabelCabecalho_v7(
      labelCabecalho_v7(state || { headerOptions: [] }, item && item.headerKey ? item.headerKey : "")
    );

    return explicitLabel || mappedLabel;
  }

  function ordenarItensPorCabecalho_v7(items, state) {
    const sortUtils = obterSortUtilsV7();

    if (
      sortUtils &&
      typeof sortUtils.sortByGroupLabelThenItemLabel_v1 === "function"
    ) {
      const sorted = sortUtils.sortByGroupLabelThenItemLabel_v1(items, {
        getGroupLabel: function (item) {
          return obterHeaderLabelOrdenacao_v7(item, state);
        },
      ...
      return sorted.map(function (item) {
        return {
          ...item,
          headerLabel: obterHeaderLabelOrdenacao_v7(item, state) || ""
        };
      });
    }

    const source = Array.isArray(items) ? items.slice() : [];
    const safeCompare = function (a, b) {
      const left = textoSeguro_v7(a).trim();
      const right = textoSeguro_v7(b).trim();
      if (!left && !right) {
        return 0;
      }
      if (!left) {
    ...
    const sortedFallback = source.sort(function (leftItem, rightItem) {
      const headerCompare = safeCompare(
        obterHeaderLabelOrdenacao_v7(leftItem, state),
        obterHeaderLabelOrdenacao_v7(rightItem, state)
      );
      if (headerCompare !== 0) {
        return headerCompare;
      }
      return safeCompare(
        leftItem && leftItem.label ? leftItem.label : "",
        rightItem && rightItem.label ? rightItem.label : ""
      );
    });

    return sortedFallback.map(function (item, index) {
      return {
        ...item,
        __managerOrder: index + 1,
        headerLabel: obterHeaderLabelOrdenacao_v7(item, state) || ""
      };
    });
  }

  function aplicarOrdenacaoStateItens_v7(state) {
    if (!state || !Array.isArray(state.items)) {
      return false;
    }

    const previousIds = state.items.map(function (item) {
      return textoSeguro_v7(item && item.__managerId ? item.__managerId : "");
    });

    const sortedItems = ordenarItensPorCabecalho_v7(state.items, state);

    const nextIds = sortedItems.map(function (item) {
      return textoSeguro_v7(item && item.__managerId ? item.__managerId : "");
    });

    const changed = previousIds.length !== nextIds.length || previousIds.some(function (id, index) {
  ...
  function iniciarGestorV7(form) {
    ...
    const initialItems = ordenarItensPorCabecalho_v7(
      juntarItensSemDuplicados_v7(
    ...
    const manager = core.createConfigurableItemsManager_v1({
      ...
      onChange: function (context) {
        const changed = aplicarOrdenacaoStateItens_v7(context.state);

        if (changed) {
          context.manager.render();
          context.manager.syncHiddenInputs();
        }
      },
