...
def _resolve_appverbo_page_state_v1(
    *,
    ...
    clean_menu = resolve_menu_key_alias(resolved_menu) or "home"
    clean_admin_tab = str(resolved_admin_tab or "").strip().lower()
    clean_profile_tab = str(resolved_profile_tab or "").strip().lower()
    clean_user_view = str(user_view or "").strip().lower()
    clean_entity_view = str(entity_view or "").strip().lower()
    clean_target = str(clean_target_from_query or "").strip()
    clean_profile_section = str(clean_profile_section_from_query or "").strip().lower()
    clean_dynamic_section = str(clean_dynamic_section_from_query or "").strip()
    clean_sidebar_section_edit_key = str(sidebar_section_edit_key or "").strip().lower()

    explicit_user_action = (
        clean_menu == "administrativo"
        and clean_admin_tab == "utilizador"
        and parsed_user_edit_id is not None
        and clean_user_view in {"0", "1", "true", "false", "sim", "nao", "não", "yes", "no", "on", "off"}
    )

    explicit_entity_action = (
        clean_menu == "administrativo"
        and clean_admin_tab == "entidade"
        and parsed_entity_edit_id is not None
        and clean_entity_view in {"0", "1", "true", "false", "sim", "nao", "não", "yes", "no", "on", "off"}
    )

    explicit_settings_action = bool(clean_settings_edit_key or clean_settings_action in {"toggle", "edit", "delete", "create"})
    explicit_sidebar_action = bool(clean_sidebar_section_edit_key)
    explicit_section_action = bool(clean_profile_section or clean_dynamic_section)

    preserve_on_browser_refresh = bool(
        is_post_save_return
        or explicit_user_action
        or explicit_entity_action
        or explicit_settings_action
        or explicit_sidebar_action
        or explicit_section_action
    )

    return {
        "version": "APPVERBO_PAGE_STATE_RESOLVER_V1",
        "refresh_home_url": "/users/new?menu=home",
        "refresh_home_menu": "home",
        "refresh_home_target": "#home-summary-card",
        "current_menu": clean_menu,
        "current_admin_tab": clean_admin_tab,
        "current_profile_tab": clean_profile_tab,
        "current_target": clean_target,
        "is_post_save_return": bool(is_post_save_return),
        "explicit_user_action": explicit_user_action,
        "explicit_entity_action": explicit_entity_action,
        "explicit_settings_action": explicit_settings_action,
        "explicit_sidebar_action": explicit_sidebar_action,
        "explicit_section_action": explicit_section_action,
        "preserve_on_browser_refresh": preserve_on_browser_refresh,
    }
# APPVERBO_PAGE_STATE_RESOLVER_V1_END
...
def new_user_page(
    ...
    page_state = _resolve_appverbo_page_state_v1(
        resolved_menu=resolved_menu,
        resolved_admin_tab=resolved_admin_tab,
        resolved_profile_tab=resolved_profile_tab,
