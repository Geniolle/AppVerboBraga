...
// APPVERBO_UTILIZADOR_EXIBIR_TOGGLE_V7_START
(function () {
  "use strict";

  if (window.__appverboUtilizadorExibirToggleV7Loaded) {
    return;
  }

  window.__appverboUtilizadorExibirToggleV7Loaded = true;

  const VIEW_PARAM = "user_view";
  const EDIT_ID_PARAM = "user_edit_id";
  const PANEL_MARKER = "data-appverbo-user-view-panel-v7";

  function normalize_v7(value) {
    return String(value || "")
      .replace(/\s+/g, " ")
      .trim()
      .toLowerCase();
  ...
  function getParams_v7() {
    return new URLSearchParams(window.location.search || "");
  }

  function isUtilizadorPage_v7() {
    const params = getParams_v7();
    const path = normalize_v7(window.location.pathname);
  ...
  function scorePanel_v7(element) {
    const text = textOf_v7(element);
    let score = 0;

    if (text.indexOf("exibir utilizador") >= 0) score += 10;
    if (text.indexOf("exibir usuário") >= 0) score += 10;
    if (text.indexOf("exibir usuario") >= 0) score += 10;
    if (text.indexOf("nome completo") >= 0) score += 3;
    if (text.indexOf("data de nascimento") >= 0) score += 3;
    if (text.indexOf("sexo") >= 0) score += 2;
    if (text.indexOf("email") >= 0) score += 3;
    if (text.indexOf("telefone") >= 0) score += 3;
    if (text.indexOf("morada") >= 0) score += 2;
    if (text.indexOf("código postal") >= 0 || text.indexOf("codigo postal") >= 0) score += 2;
    if (text.indexOf("localidade") >= 0) score += 2;
    if (text.indexOf("país") >= 0 || text.indexOf("pais") >= 0) score += 2;
    if (text.indexOf("estado da conta") >= 0 || text.indexOf("estado") >= 0) score += 2;
    if (text.indexOf("fechar") >= 0) score += 1;

    return score;
  }

  function findPanelFromTitle_v7(titleElement) {
    if (!titleElement) {
      return null;
    }
    ...
    for (let level = 0; level < 20 && current && current !== document.body; level += 1) {
      const score = scorePanel_v7(current);
  ...
  function closePanel_v7(event) {
    if (event) {
      event.preventDefault();
      event.stopPropagation();
    }

    window.location.href = buildClosedUrl_v7();
  }

  function bindCloseButtons_v7(panel) {
    if (!panel || panel.dataset.appverboUserViewCloseBoundV7 === "1") {
      return;
    }
    ...
    controls.forEach(function (control) {
      const text = textOf_v7(control);

      if (text === "fechar" || text.indexOf("fechar") >= 0) {
        control.addEventListener("click", closePanel_v7);
      }
    });

    panel.dataset.appverboUserViewCloseBoundV7 = "1";
  }

  function applyUserViewVisibility_v7() {
    if (!isUtilizadorPage_v7()) {
      return;
    }

    const titleElement = findTitleElement_v7();

    if (!titleElement) {
      return;
    }

    const panel = findPanelFromTitle_v7(titleElement);
    ...
    if (isViewMode_v7()) {
      ...
      panel.dataset.appverboUserViewOpenV7 = "1";
      bindCloseButtons_v7(panel);
      return;
    }

    panel.hidden = true;
    panel.style.display = "none";
    panel.dataset.appverboUserViewOpenV7 = "0";
  }

  function scheduleApply_v7() {
    window.clearTimeout(window.__appverboUtilizadorExibirToggleTimerV7);
    window.__appverboUtilizadorExibirToggleTimerV7 = window.setTimeout(applyUserViewVisibility_v7, 80);
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", applyUserViewVisibility_v7);
  }
  else {
    applyUserViewVisibility_v7();
  }

  window.addEventListener("load", applyUserViewVisibility_v7);
  window.addEventListener("popstate", applyUserViewVisibility_v7);

  if (document.body && window.MutationObserver) {
    const observer = new MutationObserver(scheduleApply_v7);
...
// APPVERBO_UTILIZADOR_EXIBIR_TOGGLE_V7_END
