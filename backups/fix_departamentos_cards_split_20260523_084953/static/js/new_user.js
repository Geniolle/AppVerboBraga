...
const dynamicProcessHistorySplitBlockEl = document.getElementById("dynamic-process-history-split-block");
...
function renderDynamicProcessHistory(menuKey, sectionKey, sectionLabel, sectionFields, recordLabels) {
  ...
  const hideSplitHistory = () => {
    if (dynamicProcessHistorySplitBlockEl) {
      dynamicProcessHistorySplitBlockEl.style.display = "none";
    }
  ...
  const createHeadRow = (headElement, includeStateColumn) => {
    if (!headElement) {
      return;
    }
  ...
  const renderRows = (rows, bodyElement, includeStateColumn, allowDeleteForInactive) => {
    if (!bodyElement) {
      return;
    }
  ...
  if (!tableFields.length || !visibleRows.length) {
    if (showStateColumn) {
      ...
      if (dynamicProcessHistorySplitBlockEl) {
        dynamicProcessHistorySplitBlockEl.style.display = "";
      }
  ...
  if (showStateColumn) {
    ...
    createHeadRow(dynamicProcessHistoryActiveHeadEl, true);
    createHeadRow(dynamicProcessHistoryInactiveHeadEl, true);
    renderRows(activeRows, dynamicProcessHistoryActiveBodyEl, true, false);
    renderRows(inactiveRows, dynamicProcessHistoryInactiveBodyEl, true, true);
    ...
    createHeadRow(dynamicProcessHistoryHeadEl, false);
    renderRows(visibleRows, dynamicProcessHistoryBodyEl, false, true);
    if (dynamicProcessHistoryTitleEl) {
      dynamicProcessHistoryTitleEl.textContent = `Lista de ${pluralLabel} criados`;
    }
  }
  ...
  if (dynamicProcessEditToggleEl) {
    ...
    if (
      ...
      if (historyProcessMode) {
        dynamicProcessEditToggleEl.textContent = `Criar ${toSentenceCaseText(historyRecordLabels.singular || "registo").toLowerCase()}`;
      } else {
