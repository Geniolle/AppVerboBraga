...
def _normalize_sidebar_section_status_for_page_v22(
    ...
    if clean_status in {"inativo", "inactive", "0", "false", "no", "nao", "nÃ£o", "off"}:
        return "inativo"
...
def new_user_page(
    ...
    # ###################################################################################
    # A aba Administrativo -> Menu usa o alvo canÃ³nico admin-menu-card.
    # Sem esta normalizaÃ§Ã£o, a URL admin_tab=menu pode ficar sem conteÃºdo
    # porque o backend voltava para o target padrÃ£o de Entidade.
    ...
    # O bloco usa sessÃ£o prÃ³pria para ficar isolado da estrutura legada da pÃ¡gina.
