...
async def update_dynamic_process_profile(request: Request) -> RedirectResponse:
    ...
    with SessionLocal() as session:
        ...
        if clean_menu_key == "empresa":
            return RedirectResponse(
                url=_build_post_save_redirect_url_v6(
                    ...
                    menu=clean_menu_key,
                    profile_error=(
                        "Os dados da Empresa sao lidos da entidade ativa. "
                        "Use a aba Entidade para alterar estes dados."
                    ),
                ),
                status_code=status.HTTP_303_SEE_OTHER,
            )
        ...
        if missing_required_quantity_labels:
            return RedirectResponse(
                url=_build_post_save_redirect_url_v6(submitted_form, menu=clean_menu_key,
                    profile_error="Preencha os campos obrigatorios: " + ", ".join(missing_required_quantity_labels) + ".",
                ),
                status_code=status.HTTP_303_SEE_OTHER,
            )
