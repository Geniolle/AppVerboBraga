...
script_tag = '  <script src="/static/js/modules/admin_menu_scope_guard_v1.js?v=20260508-admin-menu-scope-v1" defer></script>'

# Remove referências antigas da guarda para evitar duplicidade.
text = re.sub(
    r'\s*<script\s+src="/static/js/modules/admin_menu_scope_guard_v1\.js\?v=[^"]*"\s+defer></script>\s*',
    '\n',
    text,
)

# Remove scripts que tenham sido inseridos por engano dentro do bloco title.
