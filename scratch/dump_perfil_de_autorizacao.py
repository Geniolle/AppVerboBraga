...
def run():
    with SessionLocal() as session:
        ...
        row = session.execute(text("SELECT id, menu_key, menu_label, is_active, is_deleted, menu_config FROM sidebar_menu_settings WHERE menu_key = 'perfil_de_autorizacao'")).first()
        if row:
