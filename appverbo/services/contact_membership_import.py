...
HEADER_ALIASES_V1: dict[str, tuple[str, ...]] = {
    "name": ("nome", "name"),
    "phone": ("telefone", "telemovel", "celular", "phone"),
...
def _resolve_header_indexes_v1(headers: list[str]) -> dict[str, int]:
    ...
    if missing_roles:
        raise ValueError(
            "O ficheiro deve conter os cabecalhos Nome, Telefone e Email."
        )
...
def _read_xlsx_rows_v1(raw_bytes: bytes) -> list[list[str]]:
    try:
        ...
        raise ValueError("O ficheiro XLSX enviado e invalido.") from exc
    except Exception as exc:
        raise ValueError("Nao foi possivel ler o ficheiro XLSX.") from exc
